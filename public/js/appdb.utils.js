...
/*
 * Helper for FAQs functionality
 */
appdb.utils.Faq = (function(){
	return new function(){
		this.loadedOrdering = [];
		this.loadOrdering = function(){
			appdb.utils.Faq.loadedOrdering = [];
			$(".wikitoc > ul > li").each(function(index,elem){
				appdb.utils.Faq.loadedOrdering.push($(elem).attr("id").substr(3));
			});
		};
		this.getCurrentOrdering = function(){
			var res = [];
			$(".wikitoc > ul > li").each(function(index,elem){
				res.push($(elem).attr("id").substr(3));
			});
			return res;
		};
		this.changedOrdering = function(){
			var hasChanges = false;
			$(".wikitoc > ul > li").each(function(index,elem){
				if(appdb.utils.Faq.loadedOrdering[index] != $(elem).attr("id").substr(3)){
					hasChanges = true;
					return false;
				}
			});
		...
		this.move = function(elem,up){
			var id = $(elem).attr("id").substr(3),
				cur = appdb.utils.Faq.getCurrentOrdering(),
				old = $(elem).data('oldord') || $(elem).data('ord'),
				item = null;
				
			$(".wikitoc > ul > li").removeClass("selected");
			if(up === true && cur[0]!=id){
				item = $(elem).prev("li").clone(true);
				item.data('oldord',($(elem).prev("li").data('oldord') || $(elem).prev("li").data('ord')));
				item.data('ord',$(elem).data('ord'));
				$(elem).prev("li").remove();
				$(elem).after(item);
				$(elem).data('ord',$(elem).data('ord')-1).addClass("selected");
			}else if(cur[cur.length-1]!=id){
				item = $(elem).next("li").clone(true);
				item.data('oldord',($(elem).next("li").data('oldord') || $(elem).next("li").data('ord')));
				item.data('ord',$(elem).data('ord'));
				$(elem).next("li").remove();
				$(elem).before(item);
				$(elem).data('ord',$(elem).data('ord')+1).addClass("selected");
			}
			$(elem).data('oldord',old);
			$(item).find("span.toctoolbox").remove();
			appdb.utils.Faq.makeTOCEditable(true);
		};
		this.makeTOCEditable = function(enable){
			if ( !canEditFAQs &&  enable) {
				return;
			}
			if( !enable ){
				$(".wikitoc > ul > li").find("span.toctoolbox").remove();
				$(".wikitoc > div.header > span.savecontents").hide();
				$(".wikitoc > ul").removeClass("editmode");
				return;
			}
			$(".wikitoc > ul").addClass("editmode");
			var toccount = appdb.utils.Faq.getCurrentOrdering().length;
			$(".wikitoc > ul > li").each(function(index,elem){
				if($(elem).find("span.toctoolbox").length > 0){
					$(elem).find("span.toctoolbox").remove();
				}
				var id = $(elem).attr("id");
				id = id.substr(3);
				var ord = $(elem).data('ord');
				//Make toc items selectable
				$(elem).prepend("<span class='toctoolbox' id='toolbox"+id+"' />");
				
				//Render toc items tool box
				var order = document.createElement("span"), up = document.createElement("span"), down = document.createElement("span"), add = document.createElement("span");
				$(order).addClass("order").text($(elem).data('oldord') || $(elem).data('ord'));
				$(up).append("<a href='#' title='Move item up'><img src='/images/up"+((ord=='1')?"_gray":"")+".png' alt='up'/></a>");
				...
				$(up).find("a:last").on("click", (function(_parent,_id){
					return function(e){
						...
						if(_id != cur[0]){
							appdb.utils.Faq.move(_parent,true);
							ord = $(_parent).data('neword') || $(_parent).data('ord');
							$(_parent).css({"background-color":"green"}).animate({"background-color":"#DDD"},800,function(){$(this).removeAttr("style");});
							if(appdb.utils.Faq.changedOrdering()){
								if($(".wikitoc > div.header > span.savecontents").is(":visible") === false ){
									$(".wikitoc > div.header > span.savecontents").show().css({"background-color":"#D96B00"}).animate({"background-color":"#f9f9f9"}, 3000);
								}else{
									$(".wikitoc > div.header > span.savecontents").show();
								}
							}else{
								$(".wikitoc > div.header > span.savecontents").hide();
							}
				...
				$(down).find("a:last").on("click", (function(_parent,_id){
					return function(e){
						...
						if(_id != cur[cur.length-1]){
							appdb.utils.Faq.move(_parent,false);
							ord = $(_parent).data('neword') || $(_parent).data('ord');
							$(_parent).css({"background-color":"red"}).animate({"background-color":"#DDD"},800,function(){$(this).removeAttr("style");});
							if(appdb.utils.Faq.changedOrdering()){
								if($(".wikitoc > div.header > span.savecontents").is(":visible") === false ){
									$(".wikitoc > div.header > span.savecontents").show().css({"background-color":"#D96B00"}).animate({"background-color":"#f9f9f9"}, 3000);
								}else{
		...
		this.NewFaqHandler = (function(){
			var handler = {};
			var dialog = null;
			handler.getNewContent = function(ord,when){
				ord = ord || 1;
				when = when || "after";
				var content = "<div class='newfaq'>" +
						"<div class='header'></div>" +
						"<div class='main'>"+
							"<div class='title'>"+
								"<span class='field-name'>Title: </span>" +
								"<span class='field-value'><input type='text' value='' dojoType='dijit.form.ValidationTextBox' required='true' style='width:99%;' class='title-value' /></span>" +
							"</div>" +
							"<div class='content'>" +
								"<span class='field-name'>FAQ contents: </span>" +
								"<span class='field-value'><textarea id='newfaqcontent' style='width:100%;height:300px;'></textarea></span>" +
							"</div>" +
						"</div>" +
						"<div class='saving' style='display:none'><img src='/images/ajax-loader-small.gif' style='width:12px;height:12px;border:none;'/><span>Saving ... </span></div>"+
						"<div class='footer'>"+
			...
			handler.show = function(elem){
				if(!elem){
					appdb.debug("No faq item given");
					return;
				}
				before = (typeof before === "undefined")?false:before;
				if( $(elem).data('ord') == 1 ){
					before = false;
				}else if ( $(elem).data('ord') == appdb.utils.Faq.getCurrentOrdering() ) {
					before = true;
				}
				if(handler.dialog){
					handler.dialog.destroyRecursive(true);
					handler.dialog = null;
				}
				var cnt = $(handler.getNewContent($(elem).data('ord'),(before)?"before":"after"));
				dojo.parser.parse($(cnt)[0]);
				
				dojo.connect(dijit.byNode($(cnt).find(".cancel:last")[0]),"onClick", function(){
					handler.dialog.hide();
				});
				dojo.connect(dijit.byNode($(cnt).find(".submit:last")[0]),"onClick", function(){
					var model = appdb.model.Faq();
					model.subscribe({event: "beforeupdate", callback: function(v){
						$(cnt).find(".footer").hide();
						$(cnt).find(".saving:last").show();
						window.FaqEditMode = true;
					}, caller: this}).subscribe({event: "update", callback: function(v){
						if(v.error) return;
						appdb.utils.Faq.NewFaqHandler.close();
						appdb.views.Main.showPage('about',{mainTitle:'Faq',url:'/help/faq'});
					}, caller: this}).subscribe({event: "error", callback: function(v){
						dijit.byNode($(cnt).find(".submit:last")[0]).setAttribute('disabled', false);
						$(cnt).find(".saving:last").hide();
						$(cnt).find(".footer").show();
						var err = {
							"status": "Cannot add new FAQ item",
							"description": v.error
						};
						appdb.utils.RestApiErrorHandler(err, err,err.description).show();
					}, caller: this});
				
					var question = dijit.byNode($(cnt).find(".title-value:last")[0]).get("value"),
						answer = tinyMCE.get("newfaqcontent").getContent(),
						order = $(elem).data('ord') +1;
					model.update({query:{},data: {"question": question, "answer": answer, "order": order}});
					
				});
				
				dojo.connect(dijit.byNode($(cnt).find(".title-value:last")[0]),"onKeyUp", function(){
					testValidity();
				});
				dijit.byNode($(cnt).find(".submit:last")[0]).setAttribute('disabled', true);
				
				handler.dialog = new dijit.Dialog({
					title : "Add new FAQ item",
					content : $(cnt)[0],
					style : "width:600px;min-height:445px;",
					onHide: function(){
						handler._isOpen = false;
					},
					onShow: function(){
						handler._isOpen = true;
					}
				});
				var testValidity = (function(l){
					return function(){
						if ( $.trim($(cnt).find("textarea:last").text()) === "" || $.trim(dijit.byNode($(cnt).find(".title-value:last")[0]).get("value")) === '' ){
							dijit.byNode(l).setAttribute('disabled', true);
						}else {
							dijit.byNode(l).setAttribute('disabled', false);
						}	
					};
					
				})($(cnt).find(".submit:last")[0]);
				
				$(cnt).find("textarea:last").tinymce({
					mode : "textareas",
					// General options
					theme : "advanced",
					plugins : "autolink,lists,pagebreak,style,layer,table,save,advhr,advimage,advlink,emotions,iespell,inlinepopups,insertdatetime,preview,media,searchreplace,print,contextmenu,paste,directionality,fullscreen,noneditable,visualchars,nonbreaking,xhtmlxtras,template,advlist,xhtmlxtras",
					// Theme options
			...
			handler._isOpen = false;
			handler.isOpen = function(){
				return handler._isOpen;
			};
			return {show: handler.show, close: handler.close, isOpen: handler.isOpen};
		})();
			...
			if( canEditFAQs ){
				...
				$(".wikitoc > div.header > span.editcontents > a").on("click", function(e){
					if($(this).parent().hasClass("editing")){
						...
						setTimeout(function(){appdb.utils.Faq.setupTOC();},1);
						window.FaqEditMode = false;
					}else{
				...
				$(".wikitoc > div.header > span.savecontents > a").on("click", function(e){
					...
					var model = appdb.model.FaqReorder();
					model.subscribe({event: "beforeupdate", callback: function(){
						$(this).addClass("saving");
						$(this).empty().append("<img src='/images/ajax-loader-small.gif' border='0' width='12px' height='12px' /><span> Saving </span>");
						window.FaqEditMode = true;
					}, caller: this}).subscribe({event: "update", callback: function(v){
						$(this).removeClass("saving").empty().text("Save changes");
						if( v.error ){
							return false;
						}
						appdb.utils.DataWatcher.Registry.deactivate("faqs");
						setTimeout(function(){appdb.views.Main.showPage('about',{mainTitle:'Faq',url:'/help/faq'});},1);
					}, caller: this}).subscribe({event: "error", callback: function(v){
						var err = {
							"status": "Cannot reorder FAQs",
							"description": v.error
						};
						appdb.utils.RestApiErrorHandler(err, err,err.description).show();
					...
					model.update({query: {}, data: {ordering: appdb.utils.Faq.getCurrentOrdering()}});
					e.preventDefault();
					return false;
				});
		...
		this.setup = function(){
			...
			$("li[id^='faq'] a.editfaq").on("click", function(){
				...
				//Check if element is already in edit mode
				if ( li.next().find('textarea').length > 0 || li.find("input").length > 0) return;

				//Remove metadata for last editor user
				var faqmeta = li.next().find('.faqmeta').clone(false);
				li.next().find('.faqmeta').remove();
				
				//Setup question textbox
				var qtext = question.text();
				if( dijit.byId("question"+id) ) {
					dijit.byId("question"+id).destroyRecursive(false);
				}
				var qedit = $("<input type='text' name='question"+id+"' id='question"+id+"' value='" + qtext + "' dojoType='dijit.form.TextBox' class='faqtitleinput'  />");
				question.hide();
				li.append(qedit);
				dojo.parser.parse(li[0]);
				dojo.connect(dijit.byId("question"+id),"onKeyUp",function(){
					if ( $.trim(dijit.byId("question"+id).get("value")) === "" || $.trim($('#txtarea'+id).text()) === "" ) {
						dijit.byNode($(li).next().find(".savebutton")[0]).setAttribute('disabled', true);
						return;
					}
					dijit.byNode($(li).next().find(".savebutton")[0]).setAttribute('disabled', false);
				});
				
				//Setup textarea and faqmetadata
				var txt = li.next().html();
				li.next().empty().html('<textarea style="width:100%" id="txtarea'+id+'" >'+txt+'</textarea><div style="text-align: right;"><button class="savebutton" dojoType="dijit.form.Button">Save</button><button class="cancelbutton" dojoType="dijit.form.Button">Cancel</button></div>');
				dojo.parser.parse(li.next()[0]);
				li.find("a:first").hide();
				//Cancel Action
				dojo.connect(dijit.byNode(li.next().find('.cancelbutton')[0]),"onClick", function(){
					if( dijit.byId("question"+id) ) {
						dijit.byId("question"+id).destroyRecursive(false);
					}
					question.show();
					li.next().html(txt);
					li.next().append(faqmeta);
				});
				...
				//li.next().find('.savebutton :input').on("click", function(){
					...
					var newtext = li.next().find('textarea').val();
					tinymce.execCommand('mceFocus',false,txtid);
					var n = tinyMCE.get(txtid).getContent();
					n = $("<div>" + n+ "</div>");
					...
					$.ajax({
						...
						success: function(d) {
							...
							li.next().html(newtext).append(faqmeta);
							if( dijit.byId("question"+id) ) {
								dijit.byId("question"+id).destroyRecursive(false);
							}
							li.find("a:first").remove();
							$(li).find(".faqactions").before('<a href="'+question.attr("href")+'" title="" target="_blank">' + newquestion + '</a>');
						},
						error: function(err) {
							var errh = new appdb.views.ErrorHandler();
							errh.handle({status:"Server error " + err.status, description: "An error occured while saving FAQ text.", source: err.responseText});
						}
					});
				});
				$('#txtarea'+id).tinymce({
					...
					setup: (function(l,id){
						return function(ed) {
							ed.onKeyUp.add(function(ed, e) {
								if ( $.trim($('#txtarea'+id).text()) === "" || $.trim(dijit.byId("question"+id).get("value")) === '' ){
									dijit.byNode($(l).next().find(".savebutton")[0]).setAttribute('disabled', true);
								} else {
									dijit.byNode($(l).next().find(".savebutton")[0]).setAttribute('disabled', false);
								}
							});
						};
					})(li,id)
			...
			$("li[id^='faq'] a.removefaq").on('click', function(){
				...
				if( confirm("You are about to remove a FAQ item. Once the item is removed it cannot be retrieved again. Are you sure you want to proceed with the deletion?")){
					var model = new appdb.model.Faq();
					model.subscribe({event: "beforeremove", callback: function(){
						$(this).addClass("removing");
						$(this).find(".loading").show();
						$(this).find(".remove").hide();
						$(this).parent().parent().find(".editfaq").hide();
						...
						$(this).parent().parent().find(".editfaq").show();
						return false;
					}, caller: this});
					model.remove({"id":id});
				}
			...
			$("div[id^='faq'].locked").prev().find('a.removefaq').off('click').html("<img src='/images/logout3.png' border='0' width='12px' height='12px'/>").on("click", function(e){
				...
				setTimeout((function(_this){
					return function(){
						...
						setTimeout(function(){dijit.popup.close(pu);},8000);
					};
				})(this),1);
			});
			if( !userID ) {
			...
			this.loadOrdering();
			if(canEditFAQs){
				appdb.utils.DataWatcher.Registry.activate("faqs");
			}else{
				appdb.utils.DataWatcher.Registry.deactivate("faqs");
			}
		};
		this.scrollTo = function(index){
			...
			if($("#faq"+index).length === 0 ){
				window.scroll(0,0);
				return;
			}
			var pos = $("#faq"+index).offset().top;
			pos = parseInt(pos);
			window.scroll(0,pos-70);
			$("#faq"+index+"a").stop().removeAttr("style").addClass("highlight").animate({"background-color":"white","borderTopColor":"white","borderLeftColor":"white","borderRightColor":"white","borderBottomColor":"white"},4000, (function(elem){
				return function(){
					$(elem).removeAttr("style").removeClass("highlight");
				};})("#faq"+index+"a"));
		};
		this.getPermalink = function(index){
			index = parseInt(index);
			if(appdb.config.routing.useCanonical){
				return appdb.config.endpoint.base + "pages/about/faq" + ( (isNaN(index))?"":"/"+index);	
			}
			return appdb.config.endpoint.base + "?p=" + appdb.utils.base64.encode("/help/faq" + ( (isNaN(index))?"":"/"+index) );
		};
		this.getActions = function(p){
			var res;
			if(p.length > 10 && p.substr(0,10)==="/help/faq/"){
				var index = parseInt(p.substr(10));
				res = "appdb.utils.Faq.scrollTo(" + index + ");";
			}
			return res;
		};
	};
...
appdb.utils.AlphanumericPager = appdb.ExtendClass(appdb.utils.ObserverMediator , "appdb.utils.AlphanumericPager", function(o){
	this._extModel = null;
	this._type = "application";
	this._mediator = new appdb.utils.ObserverMediator();
	this._data = [];
	this.resetPager = function(){
		this.currentSelection("");
	};
	this.load = function(filter){
		var _model = new appdb.model.AlphanumericReport({flt:(filter || "")});
		_model.subscribe({event:"select", callback: function(d){
			_model.unsubscribeAll(this);
			d.item = d.item || [];
			this.publish({event:"load", value: d});
		}, caller: this});
		_model.get();
	};
	this.currentSelection = (function(_parent){
		var _v = "";
		return function(v){
			if(v){
				if(v==='...'){
	...
	this.current = function(v){
		if(v){
			this.currentSelection(v);
			this.publish({event:"current", value: this.getPagerFilter()});
			return this;
		}
		return this.currentSelection();
	};
	this.isEnabled = (function(_parent){
		var _v = true;
		return function(v){
			if(typeof v !== "undefined"){
				_v = (v)?true:false;
	...
	this.getPagerFilter = function(){
		if(this.currentSelection()){
			return "+~" + this._type + ".name:^[" + this.currentSelection() + "]";
		}
		return "";
	};
	this.getFullFilter = function(flt){
		return flt + ' ' + this.getPagerFilter();
	};
	this._init = function(){
		o = o || {};
		this._extModel = o.model || (new appdb.model.Applications());
		this._type = o.type || "applications";
	};
	this._init();
});
