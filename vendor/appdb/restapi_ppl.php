...
class RestPplXMLParser extends RestXMLParser {
    ...
    public function parse($xml) {
		if ( ! is_null($this->_user) ) {
			$person = new \Application\Model\Researcher();
			try {
                ...
                if ( trim(strval($image->attributes(RestAPIHelper::XMLNS_XSI())->nil)) === "true" ) {
                    ...
                    if ( ! is_null($image->attributes()->type) && trim(strval($image->attributes()->type)) === "base64" ) {
                        ...
                        if (trim(parse_url(strval($image), PHP_URL_SCHEME)) == '') {
                            ...
                            if ( trim(strval($image)) != '' ) {
                                try {
                                    $person->image = pg_escape_bytea(base64_encode(file_get_contents($_SERVER['APPLICATION_PATH']."/../public/".trim(strval($image)))));
                                    $removeImageCache = true;
                                } catch (Exception $e) {
            ...
            if ( $removeImageCache === true ) if ( $person->id != '' && file_exists($_SERVER['APPLICATION_PATH'] . "/../cache/ppl-image-".$person->id.".png") ) unlink($_SERVER['APPLICATION_PATH'] . "/../cache/ppl-image-".$person->id.".png");
			$person->save();
			...
			$cts = new \Application\Model\ContactTypes();
			$cts->refresh();
			$xmli = $xml->xpath("//person:contact");
            $conts2 = new \Application\Model\Contacts();
			foreach ($xmli as $x) {
				if ( trim(strval($x)) !== '' ) { 
...
class RestPplItem extends RestResourceItem {
    ...
    protected function init() {
		$this->_logged = false;
        // must call iserIsAdmin before instanciating a \Application\Model\Researchers, so that
        // $_GET['userid'] is properly set
        $isAdmin = $this->userIsAdmin();
		$this->_resParent = new \Application\Model\Researchers();
...
class RestDelPplList extends RestROAdminResourceList {
     ...
     */
	protected function getModel() {
		$res = new \Application\Model\Researchers();
