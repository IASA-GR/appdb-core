...
class RestAppXMLParser extends RestXMLParser {
	...
	public function parse($xml) {
        ...
        if ( ! is_null($this->_user) ) {
			$app = new \Application\Model\Application();
			try {
			...
			try {
				...
				if (strpos($e->getMessage(), "APPDB_REST_API_ERROR") === false) {
					...
					if (! is_null($errmsg)) {
						error_log('HERE!');
						$errmsg = preg_replace('/CONTEXT:.*/', '', $errmsg);
						$this->_extError = strip_tags($errmsg);
					}
				}
				return $app;
			}
			...
			if ( $this->_user->privs->canModifyApplicationLogo($app) ) {
					...
					if ( $logo->attributes(RestAPIHelper::XMLNS_XSI())->nil === "true" ) {
						...
						if ( $logo->attributes()->type && strval($logo->attributes()->type) === "base64" ) {
							...
							if (parse_url(strval($logo), PHP_URL_SCHEME) == '') {
								...
								if ( strval($logo) != '' ) {
									try {
										$app->logo = pg_escape_bytea(base64_encode(file_get_contents($_SERVER['APPLICATION_PATH']."/../public/".strval($logo)))); 
										$removeLogoCache = true;
									} catch (Exception $e) {
										$this->_error = RestErrorEnum::RE_BACKEND_ERROR;
				...
				if ( $removeLogoCache === true ) {
					$logocachename = $_SERVER['APPLICATION_PATH'] . "/../cache/app-logo-".$app->id.".png";
		...
		error_log((($TTT2 - $TTT1) - ($TTTx2 - $TTTx1)) . "s in PHP for relations and logo, " . ($TTTx2 - $TTTx1) . "s in PL/pgSQL for everything else");
...
//		return new XMLFragmentRestResponse($ret, $_this);
    ...
    protected function getModel() {
		// must call iserIsAdmin before instanciating a \Application\Model\Applications, so that
		// $_GET['userid'] is properly set
		$isAdmin = $this->userIsAdmin();
		$res = new \Application\Model\Applications();
...
/*
		$ids = array();
		$f = new \Application\Model\MailSubscriptions();
 ...
 */
	...
	public function put() {
		if ( parent::put() !== false ) {
			$bm = new \Application\Model\MailSubscription();
			$id = $this->_parser->getID($this->getData(),"application:application");
			if ( $this->_parser->getError() === RestErrorEnum::RE_OK ) {
				$apps = new \Application\Model\Applications();
				$apps->filter->id->equals($id);
...
class RestAppFollowedItem extends RestResourceItem {
	...
	public function delete() {
		if ( parent::delete() !== false ) {
			$bms = new \Application\Model\MailSubscriptions();
...
class RestAppBookmarkList extends RestResourceList {
	...
	protected function getModel() {
		$res = new \Application\Model\Applications();
		$res->filter = FilterParser::getApplications($this->getParam("flt"));
		$f = new \Application\Model\AppBookmarksFilter();
	...
	public function put() {
		if ( parent::put() !== false ) {
			$bm = new \Application\Model\AppBookmark();
			$id = $this->_parser->getID($this->getData(),"application:application");
			if ( $this->_parser->getError() === RestErrorEnum::RE_OK ) {
				$apps = new \Application\Model\Applications();
				$apps->filter->id->equals($id);
...
class RestAppBookmarkItem extends RestResourceItem {
	...
	public function delete() {
		if ( parent::delete() !== false ) {
			$bms = new \Application\Model\AppBookmarks();
...
//		$ff = new \Application\Model\ApplicationsFilter();
...
class RestRelAppList extends RestROResourceList {
	...
	protected function getModel() {
		...
		$res = new \Application\Model\RelatedApplications($id);
...
class RestAppItem extends RestResourceItem {
	...
	protected function init() {
		...
		// $_GET['userid'] is properly set
		$isAdmin = $this->userIsAdmin();
		$this->_resParent = new \Application\Model\Applications();
...
class RestAppRatingList extends RestROResourceList {
     ...
     */
    protected function getModel() {
		$res = new \Application\Model\AppRatings();
		$id = normalizeAppID($this);
...
class RestAppTagXMLParser extends RestXMLParser {
    public function parse($xml) {
		if ( ! is_null($this->_user) ) {
			$tag = new \Application\Model\AppTag();
			try {
				$xml = new SimpleXMLElement($xml);
			} catch (Exception $e) {
				$this->_error = RestErrorEnum::RE_INVALID_REPRESENTATION;
...
class RestAppTagList extends RestResourceList {
    ...
    public function get() {
        if ( parent::get() !== false ) {
            if ( $this->_listMode === RestListModeEnum::RL_NORMAL ) {
				$res = new \Application\Model\AppTags();
				$id = normalizeAppID($this);
                $res->filter->appid->numequals($id);
                $res->refresh();
                $xml = array();
...
class RestAppPubList extends RestResourceList {
    ...
    public function init() {
		$this->_res = new \Application\Model\Applications();
		$id = normalizeAppID($this);
    ...
    protected function _list() {
        $res = new \Application\Model\AppDocuments();
...
class RestEdtAppLogistics extends RestAppLogistics {
     ...
     */
	public function get($extraFilter = NULL) {
		$f = new \Application\Model\PermissionsFilter();
...
class RestAppVAXMLParser extends RestXMLParser {
	...
	private function VOGuidExists($guid){
		$vowideimagelists = new \Application\Model\VOWideImageLists();
		...
		$vowideimagelistimages = new \Application\Model\VOWideImageListImages();
	...
	private function isGroupNameUnique( $vmi, $vmiversion ){
		...
		$vappviews = new \Application\Model\Vappviews();
		$f1 = new \Application\Model\VappviewsFilter();
		$f2 = new \Application\Model\VappviewsFilter();
		...
		if( is_numeric($vmi->id) && intval($vmi->id) > 0 ){
			$f3 = new \Application\Model\VappviewsFilter();
	...
	private function canIncludeImageInstance($instance,$parent){
		$m = null;
		$insts = new \Application\Model\VMIinstances();
		$imageid = null;
		$imageguid = null;
		$version = null;
		//Find instance
	...
	private function needsVerification(){
		$instances = new \Application\Model\Vappviews();
		...
		switch($itemtype){
			case "va": //check: id, appid, appname
				...
				if( $appname && strlen( trim( $appname ) ) > 0 ){
					$m = new \Application\Model\Applications();
			...
			case "vaversion":  //check: id, identifier
				if( $xid ){
					$m = new \Application\Model\VAversions();
					$m->filter->id->equals($xid);
					if( count($m->items) > 0 ){
						return $m->items[0];
					}
				...
				if( $xidentifier ){
					...
					if( count($vas->items) > 0 ){
						$va = $vas->items[0];
						$m = new \Application\Model\VAversion();
						$m->guid = $va->guid;
						return $m;
					}
				}
				return new \Application\Model\VAversion();
				//virtualization:appliance/virtualization:instance/
			case "vmi": //check: id, identifier
				if( $xid ){
					$m = new \Application\Model\VMIs();
					$m->filter->id->equals($xid);
					if( count($m->items) > 0 ){
						return $m->items[0];
					}
				...
				return new \Application\Model\VMI();
				//virtualization:appliance/virtualization:instance/virtualization:image
			case "vmiinstance"://check: id, identifier
				if( $xid ){
					$m = new \Application\Model\VMIinstances();
					$m->filter->id->equals($xid);
					if( count($m->items) > 0 ){
						return $m->items[0];
					}
				}
				return new \Application\Model\VMIinstance();
			...
			case "vmiflavour"://check: flavourid
				...
				if( is_numeric($id) && intval($id) > 0 ){
					$m = new \Application\Model\VMIflavourBase();
					$m->filter->id->equals($id);
					if( count($m->items) > 0 ){
						return $m->items[0];
					}
				}
				return new \Application\Model\VMIflavourBase();
				//virtualization:appliance/virtualization:instance/virtualization:image/virtualization:instance
			case "arch": //check: id
				...
				return new \Application\Model\Arch();
			...
			case "os"://check: id
				...
				return new \Application\Model\OS();
			...
			case "hypervisor"://check: id
				...
				if( count($xp) > 0 ){
					...
					$id = strval($xp->attributes()->id);
					$val = trim(strval($xp));
					$m = new \Application\Model\Hypervisors();
					if( is_numeric($id) && intval($id) > 0 ){
						$m->filter->id->equals($id);
					}elseif($val !== ""){
				...
				return new \Application\Model\Hypervisor();
	...
	private function setupVAppList($vappversion, $vmiinstances = array()){
		...
		for( $i=0; $i<count($vmiinstances); $i+=1 ){
			...
			if( in_array($item->id, $instids) === false ){
				$listitem = new \Application\Model\VAList();
	...
	private function VMIExists($vmi){
		...
		$vmis = new \Application\Model\VMIs();
	...
	private function createVersionState($version){
		$old = new \Application\Model\VAversion();
		$old->id = $version->id;
	...
	private function createContextScript($contextscriptxml){
		...
		$contextscript = new \Application\Model\ContextScript();
	...
	private function syncContextScript($contextscriptxml, $vmiinstance){
		...
		try{
			...
			$vmiinstacnescripts = new \Application\Model\VMIinstanceContextScripts();
			...
			$user = $this->_parent->getUser();
			$vmiinstancecontextscript = new \Application\Model\VMIinstanceContextScript();
	...
	private function parseVAppFlavour($xml, $parent = null){
		...
		$flavours = new \Application\Model\VMIflavoursBase();
		$f = $flavours->filter;
		...
		if( count($flavours->items) > 0 ){
			...
			$flavour = new \Application\Model\VMIflavourBase();
	 ...
	 * Returns a \Application\Model\VMIinstance item.
	 */
	private function parseVAppImageInstance($xml, $parent = null){
		...
		if (count($cformats) > 0) {
			if ( (count($cformats) === 1) && ($cformats[0]->attributes(RestAPIHelper::XMLNS_XSI())->nil) && (strval($cformats[0]->attributes(RestAPIHelper::XMLNS_XSI())->nil) == "true") ) {
				$cfs = new \Application\Model\VMISupportedContextFormats();
				...
				foreach ($cformats as $cformat) {
					...
					if (($supported = "true") && ($fmtid != "")) {
						...
						$cf = new \Application\Model\VMISupportedContextFormat();
		...
		if ( count($xml->xpath('./virtualization:network_traffic[@xsi:nil="true"]')) != 0 ) {
			...
			foreach ($nts as $nt) {
				$mnt = new \Application\Model\VMINetworkTrafficEntry();
	 ...
	 * based on the given XML data.
	 * Returns a \Application\Model\VMI item.
	 */
	private function parseVAppImage($xml, $parent = null){
		...
		if( !$m->guid ){
			...
			if( $m->guid ){
				...
				$vminstances = new \Application\Model\VMIinstances();
		...
		$oldvmiinstances = new \Application\Model\VMIinstances();
	 ...
	 * based on the given XML data.
	 * Returns a \Application\Model\VAVersion item.
	 */
	 ...
	 * based on the given XML data.
	 * Returns a \Application\Model\VA item.
	 */
	...
	public function parse($xml) {
		...
		if( !is_null($this->_user) ){
			try {
				...
				return new \Application\Model\VA();
                        }
                        $this->_xml = $xml;
...
class RestAppVAItem extends RestResourceItem {
	...
	protected function init() {
		...
		// $_GET['userid'] is properly set
		$isAdmin = $this->userIsAdmin();
		$this->_resParent = new \Application\Model\VAs();
...
class RestAppContextScriptItem extends RestResourceItem{
	...
	protected function init() {
		...
		$m = new \Application\Model\ContextScripts();
...
class RestAppVAVersionItem extends RestAppVAItem {
	...
	protected function init() {
		// must call iserIsAdmin before instanciating a \Application\Model\Applications, so that
		// $_GET['userid'] is properly set
		$this->_resParent = new \Application\Model\VAversions();
	...
	private function deleteContextFormats($vmiinstanceid) {
		$scriptids = array();
		$vmicfs = new \Application\Model\VMISupportedContextFormats();
