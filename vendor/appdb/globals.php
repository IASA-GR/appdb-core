...
error_log("LOADING globals.php" . (isset($_SERVER['REFERER']) ? " [REFERER: " . $_SERVER['REFERER'] . "]" : ""));

$G_APPDB_ADAPTER;

function &APPDB_ADAPTER() {
	if (! isset($G_APPDB_ADAPTER)) {
		$G_APPDB_ADAPTER = new \Zend\Db\Adapter\Adapter(array(
			"driver" => ApplicationConfiguration::resources("db.adapter"),
			"database" => ApplicationConfiguration::resources("db.params.dbname"),
			"hostname" => ApplicationConfiguration::resources("db.params.host"),
			"username" => ApplicationConfiguration::resources("db.params.username"),
			"password" => ApplicationConfiguration::resources("db.params.password"),
			"port" => ApplicationConfiguration::resources("db.params.port")
		));	
	}
	return $G_APPDB_ADAPTER;
}
...
$_SERVER["validation_period"] = ApplicationConfiguration::app("invalid");
$_SERVER['APILatestVersion'] = ApplicationConfiguration::api("latestVersion");
$_SERVER['Repository_Enabled'] = ( ( is_null(ApplicationConfiguration::app("enableRepository")) && ApplicationConfiguration::app("enableRepository") == 'true' ) ? "true" : "false");
...
// compatibility function to replace ZF1 $this->_helper->viewRenderer->setNoRender() functionality
function &SET_NO_RENDER(&$controller, $content = NULL, $statusCode = 200) {
...
// compatibility function to replace ZF1 $this->_helper->layout->disableLayout() functionality
function &DISABLE_LAYOUT(&$controller = NULL, $noRender = false) {
	$hasView = false;
	if (! is_null($controller)) {
		if (property_exists($controller, 'view')) {
			if (! is_null($controller->view)) {
				$hasView = true;
	...
	if ((! is_null($controller)) && $noRender) {
		return SET_NO_RENDER($controller);
	} else {
	    return $result;
	}
}
...
function getZendSelectParts($select, &$from, &$where, &$orderby, &$limit) {
	$from = '';
		try {
			$from = 'FROM ' . $select->getRawState('table');
			$_from = $select->getRawState('joins')->getJoins();
			foreach ($_from as $f) {
				$from = $from . ' ' . strtoupper($f['type']) . ' JOIN ' . $f['name'] . ($f['on']===null?'':' ON '.$f['on']);
			}
		} catch (Exception $e) {
		}
		...
		$where = '';
		try {
			$wheredata = $select->getRawState('where')->getExpressionData();
			if ( count($wheredata) > 0 ) {
				$wheredata = $wheredata[0];
				if ( is_array($wheredata[0]) ) {
					foreach ($wheredata[0] as $w) {
						debug_log($w);
					}
				} else {
					$where = "WHERE " . $wheredata[0];
				};
			} else $where = '';
		} catch (Exception $e) {
		}
		...
		$orderby = '';
		try {
			$_orderby = $select->getRawState('order'); 
			if (! is_array($_orderby)) $_orderby = array($_orderby);
...
function debug_log($s) {
    if ( $_SERVER['APPLICATION_ENV'] != "production" ) error_log($s);
}
...
function validateFilterActionHelper($flt, $type, $ver = null) {
	if ( is_null($ver) ) {
		$ver = ApplicationConfiguration::api("latestVersion");
	}
	$origFlt = $flt;
	$error = '';
	$stype = '';
		...
		foreach ($a as $i) {
			...
			if ($n === 0) {
                switch ($i) {
					...
					case("sender"):
					case("person"):
						$fltstr = "id:numeric firstname:string lastname:string name:string registeredon:datetime institute:string activated:boolean lastupdated:datetime nodissemination:boolean role:string roleid:numeric language:string os:string arch:string phonebook:string license:complex accessgroup:complex";
						if ( $base === $i ) {
							$fltstr = $fltstr." country:complex application:complex middleware:complex discipline:complex category:complex";
						}
						$end = '>'.FilterParser::fieldsToXML($fltstr, $base, $n+1).'</filter:field>';
                        break;
                    case("category"):
			...
			foreach($fltarray as $fltstr) {
				...
				if ($fltstr->field === null) {
					...
					case "application":
						...
						case "relatedto":
							$f1 = false;
							$f2 = new \Application\Model\ApplicationsFilter();
							$val = str_replace("%", "", $val);
						...
						case "phonebook":
							...
							if ( (strtolower($val) >= "a") && (strtolower($val) <= "z") ) {
								...
								$f3 = new \Application\Model\ApplicationsFilter();
						...
						case "keyword":
							...
							break;
						case "metatype":
							$f1 = new \Application\Model\ApplicationsFilter();
							$f1->metatype->numequals($val);
							break;
						}
						if ($f1 === null) $f1 = new \Application\Model\ApplicationsFilter();
						break;
					case "vo":
						$f1 = null;
						switch ($fld) {
...
class ApplicationMessage {
	...
	public static function sendMessage($appid,$userid,$recipientid,$message){
		...
		//Get application
		$applications = new \Application\Model\Applications();
...
function getAlphnumericReport($type = 'applications', $flt = "", $subtype = null){
	...
	switch($type){
		...
		default:
			...
			$model = new \Application\Model\Applications();
		...
		for($i=0; $i<count($images); $i+=1){
			...
			switch($image["status"]){
				...
				case "success":
					...
					$instance->integrityMessage = "current";
					if (trim($instance->checksum) === "" || intval($instance->size) === 0) {
...
class SamlAuth{
	...
	public static function getCurrentAccount($session){
		$res = self::_getCurrentAccount($session, false);
		if (is_null($res)) {
			$res = self::_getCurrentAccount($session, true);
		}
		return $res;
	}
	private static function _getCurrentAccount($session, $doEscape){
...
class AccountConnect {
	...
	public static function isConnected($session){
		$res = self::_isConnected($session, false);
		if (is_null($res)) {
			$res = self::_isConnected($session, true);
		}
		return $res;
	}
	private static function _isConnected($session, $doEscape){
	...
	public static function isConnectedTo($session,$uid,$accounttype){
		$res = self::_isConnectedTo($session, $uid, $accounttype, false);
		if (is_null($res)) {
			$res = self::_isConnectedTo($session, $uid, $accounttype, true);
		}
		return $res;
	}
	private static function _isConnectedTo($session,$uid,$accounttype, $doEscape){
	...
	private static function getPendingConnection($accountuid, $accounttype){
		$res = self::_getPendingConnection($accountuid, $accounttype, false);
		if (is_null($res)) {
			$res = self::_getPendingConnection($accountuid, $accounttype, true);
		}
		return $res;
	}
...
//			$message .= "    " . $notification["expired"] . " image" . ( ($notification["expired"]>1)?"s":"" ) . " from an expired virtual appliance version\n";
	...
	private static function getVOObsoleteNotification($data){
		...
		try{
			...
			$notification["deletedVAs"] = $deletedVAs;
			$notification["expiredVAs"] = $expiredVAs;
			$notification["outdatedVAs"] = $outdatedVAs;
		}catch(Exception $ex){
			return null;
		}
		//Retrieve vo emails
...
class VAProviders{
	private static function getVAppliance($app){
		if( is_numeric($app) ){
			$vapps = new \Application\Model\Applications();
			$vapps->filter->id->numequals(intval($app));
			if( count($vapps->items) > 0 ){
				return $vapps->items[0];
			}
		}else if( $app instanceof \Application\Model\Application){
			return $app;
...
#		$filename = "../public/gocdbsites" . $now . ".xml";
		$filename = $_SERVER['APPLICATION_PATH'] . "/../cache/gocdbsites" . $now . ".xml";
		return $filename;
	}
