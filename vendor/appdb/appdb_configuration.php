...
require_once('Zend/Config/Reader/ReaderInterface.php');
require_once('Zend/Config/Reader/Ini.php');
...
class ApplicationConfigurationIni
{
	...
	public static function _init($options = NULL)
	{
		...
		if( $options === NULL ) {
			self::$_conf = new \Zend\Config\Reader\Ini();
			self::$_conf = self::$_conf->fromFile(__DIR__ . '/../../module/Application/config/application.ini', $_SERVER['APPLICATION_ENV']);
		} else {
			self::$_conf = json_decode(json_encode($options), true);
		}
	...
	public static function getConfig($path, $default = NULL) {
		...
		if( isset(self::$_cache[$path]) ) {
			...
			$val = self::$_conf[$_SERVER['APPLICATION_ENV']];
			$keys = explode('.', $path);
			foreach ($keys as $key) {
	...
	{
		return self::$_conf[$_SERVER['APPLICATION_ENV']][$namespace];
	}
}
...
{
	/**
	 * Retrieve values by key under the 'resources' namespace. 
	 * Eg app.wiki='https://....'
	 * 
	 * @param String $key - The key to search app namespace
	 * @param Any $default - Default value when key does not exist
	 * @return String
	 */
	public static function resources($key, $default = NULL)
	{
		return ApplicationConfigurationIni::getConfigNS('resources', $key, $default);		
	}
	 ...
	 * @return String
	 */
	public static function aaimp($key, $default = NULL)
	{
		return ApplicationConfigurationIni::getConfigNS('aaimp', $key, $default);		
	}
