	...
	*/
	...
	public function sync() {
		...
		try {
			...
			@curl_close($ch);
			@exec("rm ". APPLICATION_PATH . "/../cache/nova_providers.xml.old");
			@exec("cp " . APPLICATION_PATH . "/../cache/nova_providers.xml ". APPLICATION_PATH . "/../cache/nova_providers.xml.old");
			...
			if (@md5_file(APPLICATION_PATH . "/../cache/nova_providers.xml") !== @md5_file(APPLICATION_PATH . "/../cache/nova_providers.xml.old")) {	
				...
				if (count($rows) > 0) {
					...
					foreach($rows as $row) {
						if (trim($row->HOST_OS) != '') {
							db()->query("INSERT INTO oses (name) SELECT ? WHERE NOT EXISTS (SELECT 1 FROM oses WHERE LOWER(name) = LOWER(?))", array(trim($row->HOST_OS), trim($row->HOST_OS)));
						}
