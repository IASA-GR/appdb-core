...
<!--
	Document   : site_sensitive.xsl
	Created on : July 25, 2014, 3:56 PM
	Author     : nakos
	Description:
...
xmlns:siteservice="http://appdb.egi.eu/api/1.0/site">
	...
	<xsl:template match="//site:contact">
		<xsl:choose>
		<xsl:when test="@contacttype='email'">
			<xsl:copy>
				<xsl:copy-of select="@*" />
				<xsl:attribute name="protected">
					<xsl:value-of select="'true'"/>
				</xsl:attribute>
                <xsl:value-of select="concat('http://',//appdb:appdb/@host,'/texttoimage/sitecontact?type=',@type, '&amp;id=', ../@id)"/>
			</xsl:copy>
		</xsl:when>
		<xsl:otherwise>
		<xsl:copy>
