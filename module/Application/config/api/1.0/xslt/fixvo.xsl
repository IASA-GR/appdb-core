    ...
    <xsl:template match="IDCard">
        <xsl:element name="vo:vo">
            ...
            <xsl:if test="./Middlewares/@ARC=1">
                <xsl:element name="middleware:middleware">
                <xsl:attribute name="id">2</xsl:attribute>ARC</xsl:element>
            </xsl:if>
            <xsl:if test="./Middlewares/@gLite=1">
                <xsl:element name="middleware:middleware"><xsl:attribute name="id">1</xsl:attribute>gLite</xsl:element>
            </xsl:if>
            <xsl:if test="./Middlewares/@UNICORE=1">
                <xsl:element name="middleware:middleware"><xsl:attribute name="id">3</xsl:attribute>UNICORE</xsl:element>
            </xsl:if>
            <xsl:if test="./Middlewares/@GLOBUS=1">
                <xsl:element name="middleware:middleware"><xsl:attribute name="id">4</xsl:attribute>Globus</xsl:element>
            </xsl:if>
        </xsl:element>
    </xsl:template>
    <xsl:template match="*" />
