...
class TexttoimageController extends AbstractActionController
{
    ...
    public function personcontactAction()
    {
        ...
        $c->filter->id->equals(GET_REQUEST_PARAM($this, 'id'));
    ...
    public function indexAction()
    {
        ...
        $this->_helper->viewRenderer->setNoRender();
        $text = new textPNG;
		$text->msg = GET_REQUEST_PARAM($this, 'msg');
		if ( GET_REQUEST_PARAM($this, 'b64') == "1" ) $text->msg = base64_decode($text->msg);
        $text->size = '9';
        $this->view->text = $text;
        $text->draw();
    }
	...
	public function sitecontactAction(){
		...
		$c->filter->id->equals(GET_REQUEST_PARAM($this, "id"));
		if( count($c->items) > 0 ){
			$field = GET_REQUEST_PARAM($this, "type") . "email";
