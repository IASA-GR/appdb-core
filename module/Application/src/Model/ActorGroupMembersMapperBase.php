...
class Default_Model_ActorGroupMembersMapperBase
{
	protected $_dbTable;
	...
	public function getDbTable()
	{
		if (null === $this->_dbTable) {
			$this->setDbTable('Default_Model_DbTable_ActorGroupMembers');
		}
	...
	public function save(Default_Model_ActorGroupMember $value)
	{
		...
		if ( ! isnull($value->getGroupID()) ) $data['groupid'] = $value->getGroupID();
		if ( ! isnull($value->getActorGUID()) ) $data['actorid'] = $value->getActorGUID();
		if ( ! isnull($value->getPayload()) ) $data['payload'] = $value->getPayload();
	...
	public function delete(Default_Model_ActorGroupMember $value)
	{
		$q1 = 'id = ?';
		$q2 = $value->id;
		$s=$this->getDbTable()->getAdapter()->quoteInto($q1,$q2);
	...
	public function populate(&$entry,$row)
	{
		...
		$entry->setActorGUID($row->actorid);
		$entry->setPayload($row->payload);
	}

	public function find($id, Default_Model_ActorGroupMembers &$value)
	{
