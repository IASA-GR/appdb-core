...
class Default_Model_AppModInfosMapperBase
{
	protected $_dbTable;
	...
	public function getDbTable()
	{
		if (null === $this->_dbTable) {
			$this->setDbTable('Default_Model_DbTable_AppModInfos');
		}
		return $this->_dbTable;
	}

	public function save(Default_Model_AppModInfo $value)
	{
		global $application;
		$data = array();
		if ( ! isnull($value->getId()) ) $data['id'] = $value->getId();
		if ( ! isnull($value->getAppID()) ) $data['appid'] = $value->getAppID();
		if ( ! isnull($value->getModdedBy()) ) $data['moddedby'] = $value->getModdedBy();
		if ( ! isnull($value->getModdedOn()) ) $data['moddedon'] = $value->getModdedOn();
		if ( ! isnull($value->getModReason()) ) $data['modreason'] = $value->getModReason();
	...
	public function delete(Default_Model_AppModInfo $value)
	{
		$q1 = 'id = ?';
		$q2 = $value->id;
		$s=$this->getDbTable()->getAdapter()->quoteInto($q1,$q2);
	...
	public function populate(&$entry,$row)
	{
		...
		$entry->setModdedBy($row->moddedby);
		$entry->setModdedOn($row->moddedon);
		$entry->setModReason($row->modreason);
	}

	public function find($id, Default_Model_AppModInfos &$value)
	{
	...
	public function fetchAll($filter = null, $format = '')
	{
		...
		foreach ($resultSet as $row) {
			$entry = new Default_Model_AppModInfo();
