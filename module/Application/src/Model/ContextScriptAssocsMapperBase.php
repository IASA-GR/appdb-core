...
class Default_Model_ContextScriptAssocsMapperBase
{
	protected $_dbTable;
	...
	public function getDbTable()
	{
		if (null === $this->_dbTable) {
			$this->setDbTable('Default_Model_DbTable_ContextScriptAssocs');
		}
	...
	public function save(Default_Model_ContextScriptAssoc $value)
	{
		...
		if ( ! isnull($value->getContextID()) ) $data['contextid'] = $value->getContextID();
		if ( ! isnull($value->getContextscriptID()) ) $data['scriptid'] = $value->getContextscriptID();
	...
	public function delete(Default_Model_ContextScriptAssoc $value)
	{
		$q1 = 'id = ?';
		$q2 = $value->id;
		$s=$this->getDbTable()->getAdapter()->quoteInto($q1,$q2);
	...
	{
		$entry->setId($row->id);
		$entry->setContextID($row->contextid);
		$entry->setContextscriptID($row->scriptid);
		$entry->setAddedbyID($row->addedby);
		$entry->setAddedon($row->addedon);
	}

	public function find($id, Default_Model_ContextScriptAssocs &$value)
	{
	...
	public function fetchAll($filter = null, $format = '')
	{
		...
		foreach ($resultSet as $row) {
			$entry = new Default_Model_ContextScriptAssoc();
