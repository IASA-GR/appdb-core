...
class Default_Model_ResearchersAppsMapperBase
{
	protected $_dbTable;
	...
	public function getDbTable()
	{
		if (null === $this->_dbTable) {
			$this->setDbTable('Default_Model_DbTable_ResearchersApps');
		}
		return $this->_dbTable;
	}

	public function save(Default_Model_ResearchersApp $value)
	{
		global $application;
		$data = array();
		...
		if ( ! isnull($value->getIsKeyContact()) ) $data['iskeycontact'] = $this->pgBool($value->getIsKeyContact());
	...
	public function delete(Default_Model_ResearchersApp $value)
	{
		$q1 = array('appid = ?', 'researcherid = ?');
		$q2 = array($value->appid, $value->researcherid);
		$s = array();
	...
	public function populate(&$entry,$row)
	{
		...
		$entry->setIsKeyContact($row->iskeycontact);
		$entry->setGuID($row->guid);
	}

	public function find($id, Default_Model_ResearchersApps &$value)
	{
	...
	{
		$select = $this->getDbTable()->select();
		$select->from($this->getDbTable(),array('COUNT(DISTINCT (appid, researcherid)) AS count'));
	...
	public function fetchAll($filter = null, $format = '')
	{
		...
		foreach ($resultSet as $row) {
			$entry = new Default_Model_ResearchersApp();
