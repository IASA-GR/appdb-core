...
class Default_Model_APIKeysMapperBase
{
	protected $_dbTable;
	...
	public function getDbTable()
	{
		if (null === $this->_dbTable) {
			$this->setDbTable('Default_Model_DbTable_APIKeys');
		}
	...
	public function save(Default_Model_APIKey $value)
	{
		global $application;
		$data = array();
		if ( ! isnull($value->getKey()) ) $data['key'] = $value->getKey();
		if ( ! isnull($value->getAuthMethods()) ) $data['authmethods'] = $value->getAuthMethods();
		...
		if ( ! isnull($value->getSysAccountID()) ) $data['sysaccountid'] = $value->getSysAccountID();
	...
	public function delete(Default_Model_APIKey $value)
	{
		$q1 = 'id = ?';
		$q2 = $value->id;
		$s=$this->getDbTable()->getAdapter()->quoteInto($q1,$q2);
	...
	public function populate(&$entry,$row)
	{
		...
		$entry->setAuthMethods($row->authmethods);
		$entry->setOwnerID($row->ownerid);
		$entry->setCreatedOn($row->createdon);
		$entry->setId($row->id);
		$entry->setSysAccountID($row->sysaccountid);
	}

	public function find($id, Default_Model_APIKeys &$value)
	{
	...
	public function fetchAll($filter = null, $format = '')
	{
		...
		foreach ($resultSet as $row) {
			$entry = new Default_Model_APIKey();
