...
class Default_Model_PrivilegesMapperBase
{
	protected $_dbTable;
	...
	public function getDbTable()
	{
		if (null === $this->_dbTable) {
			$this->setDbTable('Default_Model_DbTable_Privileges');
		}
	...
	public function save(Default_Model_Privilege $value)
	{
		...
		if ( ! isnull($value->getObject()) ) $data['object'] = $value->getObject();
		if ( ! isnull($value->getId()) ) $data['id'] = $value->getId();
		if ( ! isnull($value->getActor()) ) $data['actor'] = $value->getActor();
		if ( ! isnull($value->getRevoked()) ) $data['revoked'] = $this->pgBool($value->getRevoked());
	...
	public function delete(Default_Model_Privilege $value)
	{
		$q1 = 'id = ?';
		$q2 = $value->id;
		$s=$this->getDbTable()->getAdapter()->quoteInto($q1,$q2);
	...
	public function populate(&$entry,$row)
	{
		...
		$entry->setObject($row->object);
		$entry->setId($row->id);
		$entry->setActor($row->actor);
		$entry->setRevoked($row->revoked);
	}

	public function find($id, Default_Model_Privileges &$value)
	{
	...
	public function fetchAll($filter = null, $format = '')
	{
		...
		foreach ($resultSet as $row) {
			$entry = new Default_Model_Privilege();
