...
class Default_Model_UserRequestsMapperBase
{
	protected $_dbTable;
	...
	public function getDbTable()
	{
		if (null === $this->_dbTable) {
			$this->setDbTable('Default_Model_DbTable_UserRequests');
		}
		return $this->_dbTable;
	}

	public function save(Default_Model_UserRequest $value)
	{
		global $application;
		$data = array();
		if ( ! isnull($value->getId()) ) $data['id'] = $value->getId();
		if ( ! isnull($value->getTypeID()) ) $data['typeid'] = $value->getTypeID();
		if ( ! isnull($value->getUserGUID()) ) $data['userguid'] = $value->getUserGUID();
		if ( ! isnull($value->getUserData()) ) $data['userdata'] = $value->getUserData();
		if ( ! isnull($value->getTargetGUID()) ) $data['targetguid'] = $value->getTargetGUID();
		if ( ! isnull($value->getActorGUID()) ) $data['actorguid'] = $value->getActorGUID();
		if ( ! isnull($value->getActorData()) ) $data['actordata'] = $value->getActorData();
		if ( ! isnull($value->getStateID()) ) $data['stateid'] = $value->getStateID();
		if ( ! isnull($value->getCreated()) ) $data['created'] = $value->getCreated();
	...
	public function delete(Default_Model_UserRequest $value)
	{
		$q1 = 'id = ?';
		$q2 = $value->id;
		$s=$this->getDbTable()->getAdapter()->quoteInto($q1,$q2);
	...
	public function populate(&$entry,$row)
	{
		...
		$entry->setUserGUID($row->userguid);
		$entry->setUserData($row->userdata);
		$entry->setTargetGUID($row->targetguid);
		$entry->setActorGUID($row->actorguid);
		$entry->setActorData($row->actordata);
	...
	public function find($id, Default_Model_UserRequests &$value)
	{
