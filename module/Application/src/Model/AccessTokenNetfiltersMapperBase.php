...
class Default_Model_AccessTokenNetfiltersMapperBase
{
	protected $_dbTable;
	...
	public function getDbTable()
	{
		if (null === $this->_dbTable) {
			$this->setDbTable('Default_Model_DbTable_AccessTokenNetfilters');
		}
	...
	public function save(Default_Model_AccessTokenNetfilter $value)
	{
		...
		if ( ! isnull($value->getTokenID()) ) $data['tokenid'] = $value->getTokenID();
	...
	public function delete(Default_Model_AccessTokenNetfilter $value)
	{
		$q1 = array('tokenid = ?', 'netfilter = ?');
		$q2 = array($value->tokenid, $value->netfilter);
		$s = array();
	...
	public function populate(&$entry,$row)
	{
		...
		$entry->setTokenID($row->tokenid);
	}

	public function find($id, Default_Model_AccessTokenNetfilters &$value)
	{
	...
	{
		$select = $this->getDbTable()->select();
		$select->from($this->getDbTable(),array('COUNT(DISTINCT (tokenid, netfilter)) AS count'));
	...
	public function fetchAll($filter = null, $format = '')
	{
		...
		foreach ($resultSet as $row) {
			$entry = new Default_Model_AccessTokenNetfilter();
