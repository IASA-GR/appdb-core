...
class Default_Model_HarvestContactPersonsMapperBase
{
	protected $_dbTable;
	...
	public function getDbTable()
	{
		if (null === $this->_dbTable) {
			$this->setDbTable('Default_Model_DbTable_HarvestContactPersons');
		}
		return $this->_dbTable;
	}

	public function save(Default_Model_HarvestContactPerson $value)
	{
		global $application;
		$data = array();
		if ( ! isnull($value->getId()) ) $data['id'] = $value->getId();
		if ( ! isnull($value->getIdentifier()) ) $data['identifier'] = $value->getIdentifier();
		if ( ! isnull($value->getFullName()) ) $data['fullname'] = $value->getFullName();
		if ( ! isnull($value->getEmail()) ) $data['email'] = $value->getEmail();
		if ( ! isnull($value->getPhone()) ) $data['phone'] = $value->getPhone();
		if ( ! isnull($value->getFax()) ) $data['fax'] = $value->getFax();
	...
	public function delete(Default_Model_HarvestContactPerson $value)
	{
		$q1 = 'id = ?';
		$q2 = $value->id;
		$s=$this->getDbTable()->getAdapter()->quoteInto($q1,$q2);
	...
	public function find($id, Default_Model_HarvestContactPersons &$value)
	{
	...
	public function fetchAll($filter = null, $format = '')
	{
		...
		foreach ($resultSet as $row) {
			$entry = new Default_Model_HarvestContactPerson();
