...
class Default_Model_UserCredentialsMapperBase
{
	protected $_dbTable;
	...
	public function getDbTable()
	{
		if (null === $this->_dbTable) {
			$this->setDbTable('Default_Model_DbTable_UserCredentials');
		}
		return $this->_dbTable;
	}

	public function save(Default_Model_UserCredential $value)
	{
		global $application;
		$data = array();
		if ( ! isnull($value->getId()) ) $data['id'] = $value->getId();
		if ( ! isnull($value->getResearcherID()) ) $data['researcherid'] = $value->getResearcherID();
		if ( ! isnull($value->getSessionID()) ) $data['sessionid'] = $value->getSessionID();
		if ( ! isnull($value->getToken()) ) $data['token'] = $value->getToken();
	...
	public function delete(Default_Model_UserCredential $value)
	{
		$q1 = 'id = ?';
		$q2 = $value->id;
		$s=$this->getDbTable()->getAdapter()->quoteInto($q1,$q2);
	...
	public function populate(&$entry,$row)
	{
		...
		$entry->setSessionID($row->sessionid);
		$entry->setToken($row->token);
		$entry->setAddedOn($row->addedon);
	}

	public function find($id, Default_Model_UserCredentials &$value)
	{
	...
	public function fetchAll($filter = null, $format = '')
	{
		...
		foreach ($resultSet as $row) {
			$entry = new Default_Model_UserCredential();
