...
class Default_Model_VOMembersMapperBase
{
	protected $_dbTable;
	...
	public function getDbTable()
	{
		if (null === $this->_dbTable) {
			$this->setDbTable('Default_Model_DbTable_VOMembers');
		}
		return $this->_dbTable;
	}

	public function save(Default_Model_VOMember $value)
	{
		global $application;
		$data = array();
		if ( ! isnull($value->getVoID()) ) $data['void'] = $value->getVoID();
		if ( ! isnull($value->getResearcherID()) ) $data['researcherid'] = $value->getResearcherID();
		if ( ! isnull($value->getMemberSince()) ) $data['member_since'] = $value->getMemberSince();
	...
	public function delete(Default_Model_VOMember $value)
	{
		$q1 = '';
		$q2 = '';
		$s=$this->getDbTable()->getAdapter()->quoteInto($q1,$q2);
	...
	public function populate(&$entry,$row)
	{
		...
		$entry->setMemberSince($row->member_since);
	}

	public function find($id, Default_Model_VOMembers &$value)
	{
	...
	public function fetchAll($filter = null, $format = '')
	{
		...
		foreach ($resultSet as $row) {
			$entry = new Default_Model_VOMember();
