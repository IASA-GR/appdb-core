...
class Default_Model_DatasetLicensesMapperBase
{
	protected $_dbTable;
	...
	public function getDbTable()
	{
		if (null === $this->_dbTable) {
			$this->setDbTable('Default_Model_DbTable_DatasetLicenses');
		}
	...
	public function save(Default_Model_DatasetLicense $value)
	{
		...
		if ( ! isnull($value->getLicenseID()) ) $data['licenseid'] = $value->getLicenseID();
		if ( ! isnull($value->getTitle()) ) $data['title'] = $value->getTitle();
		if ( ! isnull($value->getComment()) ) $data['comment'] = $value->getComment();
		if ( ! isnull($value->getLink()) ) $data['link'] = $value->getLink();
	...
	public function delete(Default_Model_DatasetLicense $value)
	{
		$q1 = 'id = ?';
		$q2 = $value->id;
		$s=$this->getDbTable()->getAdapter()->quoteInto($q1,$q2);
	...
	public function populate(&$entry,$row)
	{
		...
		$entry->setLicenseID($row->licenseid);
		$entry->setTitle($row->title);
		$entry->setComment($row->comment);
		$entry->setLink($row->link);
	}

	public function find($id, Default_Model_DatasetLicenses &$value)
	{
	...
	public function fetchAll($filter = null, $format = '')
	{
		...
		foreach ($resultSet as $row) {
			$entry = new Default_Model_DatasetLicense();
