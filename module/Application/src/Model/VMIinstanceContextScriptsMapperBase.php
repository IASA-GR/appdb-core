...
class Default_Model_VMIinstanceContextScriptsMapperBase
{
	protected $_dbTable;
	...
	public function getDbTable()
	{
		if (null === $this->_dbTable) {
			$this->setDbTable('Default_Model_DbTable_VMIinstanceContextScripts');
		}
		return $this->_dbTable;
	}

	public function save(Default_Model_VMIinstanceContextScript $value)
	{
		global $application;
		$data = array();
		if ( ! isnull($value->getId()) ) $data['id'] = $value->getId();
		if ( ! isnull($value->getVmiinstanceID()) ) $data['vmiinstanceid'] = $value->getVmiinstanceID();
		if ( ! isnull($value->getContextScriptID()) ) $data['contextscriptid'] = $value->getContextScriptID();
	...
	public function delete(Default_Model_VMIinstanceContextScript $value)
	{
		$q1 = 'id = ?';
		$q2 = $value->id;
		$s=$this->getDbTable()->getAdapter()->quoteInto($q1,$q2);
	...
	public function populate(&$entry,$row)
	{
		...
		$entry->setContextScriptID($row->contextscriptid);
		$entry->setAddedOn($row->addedon);
		$entry->setAddedByID($row->addedby);
	}

	public function find($id, Default_Model_VMIinstanceContextScripts &$value)
	{
	...
	public function fetchAll($filter = null, $format = '')
	{
		...
		foreach ($resultSet as $row) {
			$entry = new Default_Model_VMIinstanceContextScript();
