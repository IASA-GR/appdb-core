...
class Default_Model_PplApiLogMapperBase
{
	protected $_dbTable;
	...
	public function getDbTable()
	{
		if (null === $this->_dbTable) {
			$this->setDbTable('Default_Model_DbTable_PplApiLog');
		}
		return $this->_dbTable;
	}

	public function save(Default_Model_PplApiLog $value)
	{
		global $application;
		$data = array();
		if ( ! isnull($value->getId()) ) $data['id'] = $value->getId();
		if ( ! isnull($value->getPplID()) ) $data['pplid'] = $value->getPplID();
		...
		if ( ! isnull($value->getSource()) ) $data['source'] = $value->getSource();
	...
	public function delete(Default_Model_PplApiLogEntry $value)
	{
		$q1 = 'id = ?';
		$q2 = $value->id;
		$s=$this->getDbTable()->getAdapter()->quoteInto($q1,$q2);
	...
	{
		$entry->setId($row->id);
		$entry->setPplID($row->pplid);
		$entry->setTimestamp($row->timestamp);
		$entry->setResearcherID($row->researcherid);
		$entry->setSource($row->source);
		$entry->setIp($row->ip);
	}

	public function find($id, Default_Model_PplApiLog &$value)
	{
	...
	public function fetchAll($filter = null, $format = '')
	{
		...
		foreach ($resultSet as $row) {
			$entry = new Default_Model_PplApiLogEntry();
