...
?>
<?php
class Default_Model_AuthorBase
{
	protected $_mapper;
	protected $_id;
	protected $_docID;
	...
	public function __set($name,$value)
	{
		...
		if (('mapper' == $name) || !method_exists($this, $method)) {
			throw new Exception("Invalid Author property: '$name'");
		}
		if ( is_string($value) ) {
	...
	public function getDoc()
	{
		if ( $this->_doc === null ) {
			...
			if ($AppDocuments->count() > 0) $this->_doc = $AppDocuments->items[0];
		}
		return $this->_doc;
	}
	...
	public function getAuthor()
	{
		if ( $this->_author === null ) {
			...
			if ($Researchers->count() > 0) $this->_author = $Researchers->items[0];
		}
		return $this->_author;
	}
	...
	public function getMapper()
	{
		if (null === $this->_mapper) {
			$this->setMapper(new Default_Model_AuthorsMapper());
		}
		return $this->_mapper;
	}
	...
	public function toXML($recursive=false)
	{
		...
		if ( $recursive ) if ( $this->_doc === null ) $this->getDoc();
		if ( ! ($this->_doc === null) ) $XML .= $this->_doc->toXML();
		if ($this->_authorID !== null) $XML .= "<authorID>".$this->_authorID."</authorID>\n";
		if ( $recursive ) if ( $this->_author === null ) $this->getAuthor();
		if ( ! ($this->_author === null) ) $XML .= $this->_author->toXML();
