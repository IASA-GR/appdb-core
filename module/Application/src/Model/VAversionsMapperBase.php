...
class Default_Model_VAversionsMapperBase
{
	protected $_dbTable;
	...
	public function getDbTable()
	{
		if (null === $this->_dbTable) {
			$this->setDbTable('Default_Model_DbTable_VAversions');
		}
		return $this->_dbTable;
	}

	public function save(Default_Model_VAversion $value)
	{
		global $application;
		$data = array();
		...
		if ( ! isnull($value->getIsexternal()) ) $data['isexternal'] = $this->pgBool($value->getIsexternal());
	...
	public function delete(Default_Model_VAversion $value)
	{
		$q1 = 'id = ?';
		$q2 = $value->id;
		$s=$this->getDbTable()->getAdapter()->quoteInto($q1,$q2);
	...
	public function populate(&$entry,$row)
	{
		...
		$entry->setExpireson($row->expireson);
		$entry->setEnabled($row->enabled);
		$entry->setEnabledOn($row->enabledon);
		$entry->setEnabledByID($row->enabledby);
		$entry->setArchived($row->archived);
		$entry->setStatus($row->status);
		$entry->setArchivedon($row->archivedon);
		$entry->setSubmissionID($row->submissionid);
		$entry->setIsexternal($row->isexternal);
	}

	public function find($id, Default_Model_VAversions &$value)
	{
	...
	public function fetchAll($filter = null, $format = '')
	{
		...
		foreach ($resultSet as $row) {
			$entry = new Default_Model_VAversion();
