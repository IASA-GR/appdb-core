...
class Default_Model_RelationVerbsMapperBase
{
	protected $_dbTable;
	...
	public function getDbTable()
	{
		if (null === $this->_dbTable) {
			$this->setDbTable('Default_Model_DbTable_RelationVerbs');
		}
		return $this->_dbTable;
	}

	public function save(Default_Model_RelationVerb $value)
	{
		global $application;
		$data = array();
		if ( ! isnull($value->getId()) ) $data['id'] = $value->getId();
		if ( ! isnull($value->getName()) ) $data['name'] = $value->getName();
		if ( ! isnull($value->getDName()) ) $data['dname'] = $value->getDName();
		if ( ! isnull($value->getRName()) ) $data['rname'] = $value->getRName();
	...
	public function delete(Default_Model_RelationVerb $value)
	{
		$q1 = 'id = ?';
		$q2 = $value->id;
		$s=$this->getDbTable()->getAdapter()->quoteInto($q1,$q2);
	...
	public function populate(&$entry,$row)
	{
		...
		$entry->setRName($row->rname);
	}

	public function find($id, Default_Model_RelationVerbs &$value)
	{
	...
	public function fetchAll($filter = null, $format = '')
	{
		...
		foreach ($resultSet as $row) {
			$entry = new Default_Model_RelationVerb();
