...
class Default_Model_ResearcherCnamesMapperBase
{
	protected $_dbTable;
	...
	public function getDbTable()
	{
		if (null === $this->_dbTable) {
			$this->setDbTable('Default_Model_DbTable_ResearcherCnames');
		}
	...
	public function save(Default_Model_ResearcherCname $value)
	{
		...
		if ( ! isnull($value->getIsprimary()) ) $data['isprimary'] = $this->pgBool($value->getIsprimary());
		if ( ! isnull($value->getValue()) ) $data['value'] = $value->getValue();
	...
	public function delete(Default_Model_ResearcherCname $value)
	{
		$q1 = 'id = ?';
		$q2 = $value->id;
		$s=$this->getDbTable()->getAdapter()->quoteInto($q1,$q2);
	...
	public function populate(&$entry,$row)
	{
		...
		$entry->setIsprimary($row->isprimary);
		$entry->setValue($row->value);
		$entry->setResearcherID($row->researcherid);
	}

	public function find($id, Default_Model_ResearcherCnames &$value)
	{
	...
	public function fetchAll($filter = null, $format = '')
	{
		...
		foreach ($resultSet as $row) {
			$entry = new Default_Model_ResearcherCname();
