...
class Default_Model_VOsMapperBase
{
	protected $_dbTable;
	...
	public function getDbTable()
	{
		if (null === $this->_dbTable) {
			$this->setDbTable('Default_Model_DbTable_VOs');
		}
		return $this->_dbTable;
	}

	public function save(Default_Model_VO $value)
	{
		global $application;
		$data = array();
		if ( ! isnull($value->getId()) ) $data['id'] = $value->getId();
		if ( ! isnull($value->getName()) ) $data['name'] = $value->getName();
		if ( ! isnull($value->getScope()) ) $data['scope'] = $value->getScope();
		if ( ! isnull($value->getValidated()) ) $data['validated'] = $value->getValidated();
		...
		if ( ! isnull($value->getEnrollment()) ) $data['enrollment'] = $value->getEnrollment();
		if ( ! isnull($value->getAup()) ) $data['aup'] = $value->getAup();
		if ( ! isnull($value->getDomainID()) ) $data['domainid'] = $value->getDomainID();
		...
		if ( ! isnull($value->getAlias()) ) $data['alias'] = $value->getAlias();
	...
	public function delete(Default_Model_VO $value)
	{
		$q1 = 'id = ?';
		$q2 = $value->id;
		$s=$this->getDbTable()->getAdapter()->quoteInto($q1,$q2);
	...
	public function populate(&$entry,$row)
	{
		...
		$entry->setScope($row->scope);
		$entry->setValidated($row->validated);
		$entry->setDescription($row->description);
		$entry->setHomepage($row->homepage);
		$entry->setEnrollment($row->enrollment);
		$entry->setAup($row->aup);
		$entry->setDomainID($row->domainid);
	...
	public function find($id, Default_Model_VOs &$value)
	{
