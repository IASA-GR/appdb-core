...
CREATE TABLE gocdb.nova_providers (
	pkey text NOT NULL,
	...
	node_monitored bool NULL,
	sitename text NULL,
	country_name text NULL,
	country_code text NULL,
	roc_name text NULL,
	url text NULL,
	CONSTRAINT nova_providers_pkey_key UNIQUE (pkey)
);
CREATE INDEX idx_gocdb_nova_providers_hostos ON gocdb.nova_providers USING btree (host_os);
CREATE INDEX idx_gocdb_nova_providers_hostos_low ON gocdb.nova_providers USING btree (lower(host_os));
CREATE INDEX idx_gocdb_nova_providers_hostos_low_textops ON gocdb.nova_providers USING btree (lower(host_os) text_pattern_ops);
CREATE INDEX idx_gocdb_nova_providers_hostos_low_trgmops ON gocdb.nova_providers USING gin (lower(host_os) gin_trgm_ops);
CREATE INDEX idx_gocdb_nova_providers_hostos_textops ON gocdb.nova_providers USING btree (host_os text_pattern_ops);
CREATE INDEX idx_gocdb_nova_providers_hostos_trgmops ON gocdb.nova_providers USING gin (host_os gin_trgm_ops);

CREATE VIEW __nova_providers AS
SELECT
	nova_providers.pkey AS id,
    nova_providers.sitename,
    nova_providers.url,
    nova_providers.gocdb_url,
    nova_providers.hostname,
    nova_providers.host_dn,
    nova_providers.host_ip,
    oses.id AS host_os_id,
    archs.id AS host_arch_id,
    nova_providers.beta,
    nova_providers.in_production,
    nova_providers.node_monitored,
    countries.id AS country_id,
    nova_providers.roc_name AS ngi,
    uuid_generate_v5(uuid_namespace('ISO OID'::text), nova_providers.pkey) AS guid
   FROM gocdb.nova_providers
     LEFT JOIN oses ON lower(oses.name) = lower(nova_providers.host_os)
     LEFT JOIN archs ON lower(archs.name) = lower(nova_providers.host_arch) OR (lower(nova_providers.host_arch) = ANY (lower(archs.aliases::text)::text[]))
     LEFT JOIN countries ON countries.isocode = nova_providers.country_code
     JOIN gocdb.sites ON lower(sites.name) = lower(nova_providers.sitename)
  WHERE sites.certstatus <> ALL (ARRAY['Closed'::text, 'Suspended'::text]);

CREATE MATERIALIZED VIEW public.nova_providers
AS SELECT
	__nova_providers.id,
    __nova_providers.sitename,
    __nova_providers.url,
    __nova_providers.gocdb_url,
    __nova_providers.hostname,
    __nova_providers.host_dn,
    __nova_providers.host_ip,
    __nova_providers.host_os_id,
    __nova_providers.host_arch_id,
    __nova_providers.beta,
    __nova_providers.in_production,
    __nova_providers.node_monitored,
    __nova_providers.country_id,
    __nova_providers.ngi,
    __nova_providers.guid
   FROM __nova_providers
WITH DATA;

-- View indexes:
CREATE INDEX idx_nova_providers_guid ON public.nova_providers USING btree (guid);
CREATE UNIQUE INDEX idx_nova_providers_id ON public.nova_providers USING btree (id);
CREATE INDEX idx_nova_providers_sitename ON public.nova_providers USING btree (sitename);
CREATE INDEX idx_nova_providers_sitename_in_production ON public.nova_providers USING btree (sitename, in_production);
CREATE INDEX idx_nova_providers_sitename_isprod ON public.nova_providers USING btree (sitename) WHERE (in_production IS TRUE);
CREATE INDEX idx_nova_providers_sitename_textops ON public.nova_providers USING btree (sitename text_pattern_ops);
CREATE INDEX idx_nova_providers_sitename_trgmops ON public.nova_providers USING gin (sitename gin_trgm_ops);
