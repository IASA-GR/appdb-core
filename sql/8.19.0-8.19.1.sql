...
CREATE OR REPLACE FUNCTION public.relation_to_xml(relid integer, guid uuid DEFAULT NULL::uuid)
 RETURNS SETOF xml
 LANGUAGE sql
 STABLE
AS $function$SELECT XMLELEMENT(
    ...
    XMLELEMENT(
        ...
        XMLATTRIBUTES(
            ...
            CASE WHEN er.subject_type IN ('software', 'vappliance') THEN
		(SELECT applications.pidhandle FROM applications WHERE guid = er.subject_guid)
            ELSE
		NULL
            END AS handle
        ),
    ...
    XMLELEMENT(
	    ...
	    CASE WHEN er.target_type IN ('software', 'vappliance') THEN
		(SELECT applications.pidhandle FROM applications WHERE guid = er.target_guid)
            ELSE
		NULL
            END AS handle
        ),
...
GROUP BY cds.id
-- LIMIT 1 -- FIXME: properly aggregate rows when and if multiple CD types are supported. (LIMIT used to prevent possible cardinality errors)
...
INSERT INTO version (major,minor,revision,notes) 
	SELECT 8, 19, 1, E'Include Handle PIDs in extended app XML representation and entity-relation XML representation'
	WHERE NOT EXISTS (SELECT * FROM version WHERE major=8 AND minor=19 AND revision=1);
