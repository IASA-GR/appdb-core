...
FROM pidhandles
WHERE
        (entrytype = 'vappliance_version') AND
        (entryid = $1.id) AND
        ((result & 1)::BOOLEAN) AND -- marked as registered
        (NOT ((result & 8)::BOOLEAN)) -- not marked as to-be-deleted
$function$;
ALTER FUNCTION pidhandle(vapp_versions) OWNER TO appdb;

CREATE OR REPLACE FUNCTION logourl(applications)
 RETURNS text
 LANGUAGE sql
 STABLE
AS $function$
	SELECT 'https://' || (SELECT data FROM config WHERE var='ui-host') || '/apps/getlogo?id=' || $1.id::text
$function$;
ALTER FUNCTION logourl(applications) OWNER TO appdb;

INSERT INTO config (var, data)
SELECT 'latest_api', '1.0'
WHERE NOT EXISTS (SELECT 1 FROM config WHERE var = 'latest_api');

CREATE OR REPLACE FUNCTION dataurl(applications)
 RETURNS text
 LANGUAGE sql
 STABLE
AS $function$
	SELECT 'https://' || (SELECT data FROM config WHERE var='api-host') || '/rest/' || (SELECT data FROM config WHERE var = 'latest_api') || '/applications/' || $1.id::text
$function$;
ALTER FUNCTION dataurl(applications) OWNER TO appdb;

CREATE OR REPLACE FUNCTION curl(applications)
 RETURNS text
 LANGUAGE sql
 STABLE
AS $function$
	SELECT 'https://' || (SELECT data FROM config WHERE var='ui-host') || '/store/' || CASE $1.metatype WHEN 0 THEN 'software' WHEN '1' THEN 'vappliance' ELSE 'swappliance' END || '/' || $1.cname::text
$function$;
ALTER FUNCTION curl(applications) OWNER TO appdb;

CREATE OR REPLACE FUNCTION curl(researchers)
 RETURNS text
 LANGUAGE sql
 STABLE
AS $function$
	SELECT 'https://' || (SELECT data FROM config WHERE var='ui-host') || '/store/person/' || $1.cname::text
$function$;
ALTER FUNCTION curl(researchers) OWNER TO appdb;

CREATE OR REPLACE FUNCTION curl(vapp_versions)
 RETURNS TEXT
 LANGUAGE sql
 STABLE
AS $function$
	SELECT 'https://' ||
		(SELECT data FROM config WHERE var = 'ui-host') ||
		'/store/vappliance/' ||
		applications.cname ||
		'/vaversion/' ||
	...
	WHERE vapplications.id = $1.vappid
$function$;
ALTER FUNCTION curl(vapp_versions) OWNER TO appdb;

DROP VIEW IF EXISTS endorsables;
...
WHERE
	...
	(NOT vaviews.imglst_private) AND
	(NOT vaviews.va_version_publishedon IS NULL) AND
	vaviews.va_version_expireson > NOW()
;
