 ...
 SELECT __va_providers.sitename,
   ...
   FROM __va_providers
     LEFT JOIN va_provider_images ON va_provider_images.va_provider_id = __va_providers.id AND (va_provider_images.vmiinstanceid IN ( SELECT __vaviews.vmiinstanceid
           FROM __vaviews))
  GROUP BY __va_providers.id, __va_providers.hostname, __va_providers.beta, __va_providers.in_production, __va_providers.occi_downtime, __va_providers.sitename
WITH DATA;
...
SELECT xmlagg(services.x) FROM (SELECT XMLELEMENT(NAME "site:service", 
    XMLATTRIBUTES( 'occi' as type, va_providers.id as id , hostname as host, va_providers.beta as beta, va_providers.in_production as in_production, va_providers.occi_downtime::int as occi_downtime, va_providers.node_monitored as monitored, va_providers.ngi as ngi),
   ...
   va_providers.beta, va_providers.in_production, va_providers.occi_downtime, va_providers.node_monitored, 
...
INSERT INTO version (major,minor,revision,notes) 
	SELECT 8, 13, 0, E'Added bitwise column occi_downtime to va_providers'
	WHERE NOT EXISTS (SELECT * FROM version WHERE major=8 AND minor=13 AND revision=0);
