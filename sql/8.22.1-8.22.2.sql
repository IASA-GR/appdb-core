...
Previous version: 8.22.1
New version: 8.22.2
Author: wvkarag@lovecraft.priv.iasa.gr
*/

-- Function: public.filteritems(text, text, text, text)

-- DROP FUNCTION public.filteritems(text, text, text, text);
...
INSERT INTO version (major,minor,revision,notes) 
	SELECT 8, 22, 2, E'Ignore possible concurrency-related errors when creating filter cache tables'
	WHERE NOT EXISTS (SELECT * FROM version WHERE major=8 AND minor=22 AND revision=2);
