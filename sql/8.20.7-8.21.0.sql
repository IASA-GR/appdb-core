...
DROP FUNCTION IF EXISTS consume_doc(TEXT,INT);
CREATE OR REPLACE FUNCTION consume_doc(xml_in TEXT, appid_in INT) RETURNS TEXT AS
$$
DECLARE
        _docid INT;
        _appid INT;
...
BEGIN
        ...
        IF NOT _docid IS NULL THEN
                WITH x AS (
                        SELECT
                                ((xpath('./publication:title/text()', xdoc, appdb_xpathns()))[1])::TEXT AS xtitle,
                                ((xpath('./publication:url/text()', xdoc, appdb_xpathns()))[1])::TEXT AS xurl,
                                ((xpath('./publication:conference/text()', xdoc, appdb_xpathns()))[1])::TEXT AS xconference,
                                ((xpath('./publication:proceedings/text()', xdoc, appdb_xpathns()))[1])::TEXT AS xproceedings,
                                ((xpath('./publication:isbn/text()', xdoc, appdb_xpathns()))[1])::TEXT AS xisbn,
                                ...
                                ((xpath('./publication:volume/text()', xdoc, appdb_xpathns()))[1])::TEXT AS xvolume,
                                ((xpath('./publication:publisher/text()', xdoc, appdb_xpathns()))[1])::TEXT AS xpublisher,
                                ((xpath('./publication:journal/text()', xdoc, appdb_xpathns()))[1])::TEXT AS xjournal,
                                ...
                                CASE
                                        WHEN NOT (xpath('./publication:author[@main="true"][@type="internal"]', xdoc, appdb_xpathns()))[1] IS NULL THEN
                                                (SELECT name FROM researchers WHERE id = (((xpath('./publication:author[@main="true"][@type="internal"]/@id', xdoc, appdb_xpathns()))[1])::TEXT)::INT)
                                        ELSE
                                                ((xpath('./publication:author[@main="true"][@type="external"]/text()', xdoc, appdb_xpathns()))[1])::TEXT
                                END AS xmainauthor
                )
                UPDATE appdocuments
                        SET
        ...
        IF xpath_exists('./publication:author[@type="external"]', xdoc, appdb_xpathns()) THEN
                ...
                WHERE (docid = _docid) AND NOT (author = ANY(xpath('./publication:extAuthor/text()', xtmp, appdb_xpathns())));
                 --FIXME: author = ANY ... should be case-insensitive

                WITH xauth AS (
                        SELECT
                                (((xpath('./@main', x, appdb_xpathns()))[1])::TEXT)::BOOLEAN AS xmain,
                                (((xpath('./publication:extAuthor/text()', x, appdb_xpathns()))[1])::TEXT)::INT AS xauthor
                ...
                FROM xauth
                WHERE NOT (xauthor IS NULL) AND NOT EXISTS (SELECT 1 FROM extauthors WHERE (docid = _docid) AND author = xauthor);
        END IF;
        ...
        IF xpath_exists('./publication:author[@type="internal"]', xdoc, appdb_xpathns()) THEN
                ...
                WHERE (docid = _docid) AND NOT (
                        authorid = ANY(
                                ((xpath(
                                        './publication:author[@type="internal"]/@id'::TEXT,
                                        xdoc,
                                        appdb_xpathns()
                                ))::TEXT[])::INT[]
                        )
...
END;
$$ LANGUAGE plpgsql;
ALTER FUNCTION consume_doc(TEXT, INT) OWNER TO appdb;
