...
ALTER TABLE gocdb.va_providers ADD COLUMN argo_status TEXT;
ALTER TABLE gocdb.va_providers ADD COLUMN argo_status_date timestamp;
...
CREATE OR REPLACE VIEW public.__va_providers AS 
 SELECT va_providers.pkey AS id,
    ...
    va_providers.occi_downtime,
    va_providers.argo_status,
    va_providers.argo_status_date
...
CREATE MATERIALIZED VIEW public.va_providers AS 
 SELECT __va_providers.id,
    ...
    __va_providers.occi_downtime,
    __va_providers.argo_status,
    __va_providers.argo_status_date
   FROM __va_providers
WITH DATA;
...
SELECT 
	xmlelement(
		...
		xmlattributes(
			...
			in_production,
			node_monitored,
			occi_downtime::int AS occi_downtime,
			argo_status AS argo_status,
			argo_status_date AS argo_status_date
		),
 ...
 SELECT __va_providers.sitename,
   ...
   FROM __va_providers
     LEFT JOIN va_provider_images ON va_provider_images.va_provider_id = __va_providers.id AND (va_provider_images.vmiinstanceid IN ( SELECT __vaviews.vmiinstanceid
           FROM __vaviews))
  GROUP BY __va_providers.id, __va_providers.hostname, __va_providers.beta, __va_providers.in_production, __va_providers.occi_downtime, __va_providers.sitename, __va_providers.argo_status, __va_providers.argo_status_date
WITH DATA;
...
SELECT xmlagg(services.x) FROM (SELECT XMLELEMENT(NAME "site:service", 
    XMLATTRIBUTES( 'occi' as type, va_providers.id as id , hostname as host, va_providers.beta as beta, va_providers.in_production as in_production, va_providers.occi_downtime::int as occi_downtime, va_providers.argo_status, va_providers.argo_status_date, va_providers.node_monitored as monitored, va_providers.ngi as ngi),
   ...
   va_providers.beta, va_providers.in_production, va_providers.occi_downtime, va_providers.argo_status, va_providers.argo_status_date, va_providers.node_monitored, 
...
INSERT INTO version (major,minor,revision,notes) 
	SELECT 8, 13, 1, E'Added argo_status and argo_status_date to va_providers'
