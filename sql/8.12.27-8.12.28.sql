...
Previous version: 8.12.27
New version: 8.12.28
Author: wvkarag@lovecraft.priv.iasa.gr
*/
-- Function: filteritems(text, text, text, text)

-- DROP FUNCTION filteritems(text, text, text, text);
START TRANSACTION;

CREATE OR REPLACE FUNCTION filteritems(
    fltstr text,
    m_from text,
    m_where text,
    itemtype text)
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION filteritems(text, text, text, text)
  OWNER TO appdb;

INSERT INTO version (major,minor,revision,notes) 
	SELECT 8, 12, 28, E'Regression bug fix for filteritems, due to sites becoming a materialized view'
	WHERE NOT EXISTS (SELECT * FROM version WHERE major=8 AND minor=12 AND revision=28);
COMMIT;
