...
xmlns:middleware="http://appdb.egi.eu/api/1.0/middleware">
	...
	<xsl:template match="middleware:middleware" />
	...
	<xsl:template match="discipline:subdiscipline" />
	...
	<xsl:template match="application:application">
		<xsl:element name="application">
			...
			<xsl:for-each select="./middleware:middleware">
			...
			<xsl:for-each select="./discipline:discipline">
				<xsl:sort select="node()" />
				<xsl:if test="string(.) != ''">
			...
			<xsl:for-each select="./discipline:subdiscipline">
				<xsl:sort select="node()" />
				<xsl:if test="string(.) != ''">
			...
			<xsl:for-each select="./application:category">
				<xsl:sort select="node()" />
...
<!--	<xsl:template match="application:subdiscipline/@*" />
	...
	<xsl:template match="application:owner" >
		<xsl:if test="concat(./person:firstname,./person:lastname) != ''">
		<xsl:element name="owner">
			<xsl:value-of select="concat(./person:firstname,' ',./person:lastname)"/>
		</xsl:element>
		</xsl:if>
	</xsl:template>
	<xsl:template match="application:addedby">
		<xsl:if test="concat(./person:firstname,./person:lastname) != ''">
