...
class Default_Model_CountryRegionsMapperBase
{
	protected $_dbTable;
	...
	public function getDbTable()
	{
		if (null === $this->_dbTable) {
			$this->setDbTable('Default_Model_DbTable_CountryRegions');
		}
		return $this->_dbTable;
	}

	public function save(Default_Model_CountryRegion $value)
	{
		global $application;
		$data = array();
		if ( ! isnull($value->getRegionID()) ) $data['regionid'] = $value->getRegionID();
		if ( ! isnull($value->getRegion()) ) $data['region'] = $value->getRegion();
		if ( ! isnull($value->getCountryID()) ) $data['countryid'] = $value->getCountryID();
		if ( ! isnull($value->getCountry()) ) $data['country'] = $value->getCountry();
		if ( ! isnull($value->getISOcode()) ) $data['isocode'] = $value->getISOcode();
		if ( ! isnull($value->getContinent()) ) $data['continent'] = $this->pgBool($value->getContinent());
	...
	public function delete(Default_Model_CountryRegion $value)
	{
		$q1 = '';
		$q2 = '';
		$s=$this->getDbTable()->getAdapter()->quoteInto($q1,$q2);
	...
	public function populate(&$entry,$row)
	{
		...
		$entry->setRegion($row->region);
		$entry->setCountryID($row->countryid);
		$entry->setCountry($row->country);
		$entry->setISOcode($row->isocode);
		$entry->setContinent($row->continent);
	}

	public function find($id, Default_Model_CountryRegions &$value)
	{
	...
	public function fetchAll($filter = null, $format = '')
	{
		...
		foreach ($resultSet as $row) {
			$entry = new Default_Model_CountryRegion();
