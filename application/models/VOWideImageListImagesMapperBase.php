...
class Default_Model_VOWideImageListImagesMapperBase
{
	protected $_dbTable;
	...
	public function getDbTable()
	{
		if (null === $this->_dbTable) {
			$this->setDbTable('Default_Model_DbTable_VOWideImageListImages');
		}
	...
	public function save(Default_Model_VOWideImageListImage $value)
	{
		...
		if ( ! isnull($value->getVowideImageListID()) ) $data['vowide_image_list_id'] = $value->getVowideImageListID();
	...
	public function delete(Default_Model_VOWideImageListImage $value)
	{
		$q1 = 'id = ?';
		$q2 = $value->id;
		$s=$this->getDbTable()->getAdapter()->quoteInto($q1,$q2);
	...
	{
		$entry->setId($row->id);
		$entry->setVowideImageListID($row->vowide_image_list_id);
		$entry->setVappListID($row->vapplistid);
		$entry->setGuid($row->guid);
		$entry->setState($row->state);
	}

	public function find($id, Default_Model_VOWideImageListImages &$value)
	{
	...
	public function fetchAll($filter = null, $format = '')
	{
		...
		foreach ($resultSet as $row) {
			$entry = new Default_Model_VOWideImageListImage();
