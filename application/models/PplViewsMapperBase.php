...
class Default_Model_PplViewsMapperBase
{
	protected $_dbTable;
	...
	public function getDbTable()
	{
		if (null === $this->_dbTable) {
			$this->setDbTable('Default_Model_DbTable_PplViews');
		}
		return $this->_dbTable;
	}

	public function save(Default_Model_PplView $value)
	{
		global $application;
		$data = array();
		...
		if ( ! isnull($value->getImage()) ) $data['image'] = $value->getImage();
		if ( ! isnull($value->getName()) ) $data['name'] = $value->getName();
		if ( ! isnull($value->getRegionID()) ) $data['regionid'] = $value->getRegionID();
		if ( ! isnull($value->getDocCount()) ) $data['doccount'] = $value->getDocCount();
	...
	public function delete(Default_Model_PplView $value)
	{
		$q1 = '';
		$q2 = '';
		$s=$this->getDbTable()->getAdapter()->quoteInto($q1,$q2);
	...
	public function populate(&$entry,$row)
	{
		...
		$entry->setDocCount($row->doccount);
		$entry->setHasDocs($row->hasdocs);
		$entry->setGuid($row->guid);
	}

	public function find($id, Default_Model_PplViews &$value)
	{
	...
	public function fetchAll($filter = null, $format = '')
	{
		...
		foreach ($resultSet as $row) {
			$entry = new Default_Model_PplView();
