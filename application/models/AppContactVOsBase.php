...
class Default_Model_AppContactVOsBase {
	protected $_items;
	protected $_mapper;
	protected $_filter;
	protected $_format;
	...
	public function __construct($filter = null)
	{
		$this->_items = null;
		if ( $filter === null ) {
			$this->_filter = new Default_Model_AppContactVOsFilter();
		} else {
			$this->_filter = $filter;
		}
	}
	...
	public function __set($name,$value)
	{
		...
		if (('mapper' == $name) || !method_exists($this, $method)) {
				throw new Exception("Invalid AppContactVOs property: '$name'");
		}
		$this->$method($value);
	}
	...
	public function item($id)
	{
		if ($this->_items === null) $this->refresh();
		foreach ($this->_items as $item)
				if (($item->_appID == $id['appID']) && ($item->_researcherID == $id['researcherID']) && ($item->_voID == $id['voID'])) return $item;
		return null;
	}
