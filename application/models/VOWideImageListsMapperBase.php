...
class Default_Model_VOWideImageListsMapperBase
{
	protected $_dbTable;
	...
	public function getDbTable()
	{
		if (null === $this->_dbTable) {
			$this->setDbTable('Default_Model_DbTable_VOWideImageLists');
		}
		return $this->_dbTable;
	}

	public function save(Default_Model_VOWideImageList $value)
	{
		global $application;
		$data = array();
		...
		if ( ! isnull($value->getState()) ) $data['state'] = $value->getState();
		if ( ! isnull($value->getExpiresOn()) ) $data['expires_on'] = $value->getExpiresOn();
		if ( ! isnull($value->getPublishedOn()) ) $data['published_on'] = $value->getPublishedOn();
		if ( ! isnull($value->getNotes()) ) $data['notes'] = $value->getNotes();
		if ( ! isnull($value->getTitle()) ) $data['title'] = $value->getTitle();
		if ( ! isnull($value->getAlteredByID()) ) $data['alteredby'] = $value->getAlteredByID();
		if ( ! isnull($value->getLastModified()) ) $data['lastmodified'] = $value->getLastModified();
	...
	public function delete(Default_Model_VOWideImageList $value)
	{
		$q1 = 'id = ?';
		$q2 = $value->id;
		$s=$this->getDbTable()->getAdapter()->quoteInto($q1,$q2);
	...
	public function populate(&$entry,$row)
	{
		...
		$entry->setExpiresOn($row->expires_on);
		$entry->setPublishedOn($row->published_on);
		$entry->setNotes($row->notes);
		$entry->setTitle($row->title);
		$entry->setAlteredByID($row->alteredby);
		$entry->setLastModified($row->lastmodified);
		$entry->setPublishedByID($row->publishedby);
	}

	public function find($id, Default_Model_VOWideImageLists &$value)
	{
