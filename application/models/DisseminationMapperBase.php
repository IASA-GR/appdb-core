...
class Default_Model_DisseminationMapperBase
{
	protected $_dbTable;
	...
	public function getDbTable()
	{
		if (null === $this->_dbTable) {
			$this->setDbTable('Default_Model_DbTable_Dissemination');
		}
	...
	public function save(Default_Model_DisseminationEntry $value)
	{
		...
		if ( ! isnull($value->getComposerID()) ) $data['composerid'] = $value->getComposerID();
		if ( ! isnull($value->getRecipients()) ) $data['recipients'] = $value->getRecipients();
		if ( ! isnull($value->getFilter()) ) $data['filter'] = $value->getFilter();
		if ( ! isnull($value->getSubject()) ) $data['subject'] = $value->getSubject();
		if ( ! isnull($value->getMessage()) ) $data['message'] = $value->getMessage();
		if ( ! isnull($value->getSentOn()) ) $data['senton'] = $value->getSentOn();
	...
	public function delete(Default_Model_DisseminationEntry $value)
	{
		$q1 = 'id = ?';
		$q2 = $value->id;
		$s=$this->getDbTable()->getAdapter()->quoteInto($q1,$q2);
	...
	{
		$entry->setId($row->id);
		$entry->setComposerID($row->composerid);
		$entry->setRecipients($row->recipients);
		$entry->setFilter($row->filter);
		$entry->setSubject($row->subject);
		$entry->setMessage($row->message);
		$entry->setSentOn($row->senton);
	}

	public function find($id, Default_Model_DisseminationEntry &$value)
	{
