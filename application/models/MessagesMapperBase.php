...
class Default_Model_MessagesMapperBase
{
	protected $_dbTable;
	...
	public function save(Default_Model_Message $value)
	{
		...
		if ( ! isnull($value->getReceiverID()) ) $data['receiverid'] = $value->getReceiverID();
		if ( ! isnull($value->getSenderID()) ) $data['senderid'] = $value->getSenderID();
		if ( ! isnull($value->getMsg()) ) $data['msg'] = $value->getMsg();
		if ( ! isnull($value->getSentOn()) ) $data['senton'] = $value->getSentOn();
		if ( ! isnull($value->getIsRead()) ) $data['isread'] = $this->pgBool($value->getIsRead());
	...
	public function delete(Default_Model_Message $value)
	{
		$q1 = 'id = ?';
		$q2 = $value->id;
		$s=$this->getDbTable()->getAdapter()->quoteInto($q1,$q2);
	...
	{
		$entry->setId($row->id);
		$entry->setReceiverID($row->receiverid);
		$entry->setSenderID($row->senderid);
		$entry->setMsg($row->msg);
		$entry->setSentOn($row->senton);
		$entry->setIsRead($row->isread);
	}

	public function find($id, Default_Model_Messages &$value)
	{
	...
	public function fetchAll($filter = null, $format = '')
	{
		...
		foreach ($resultSet as $row) {
			$entry = new Default_Model_Message();
