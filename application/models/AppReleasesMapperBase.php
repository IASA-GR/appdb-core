...
class Default_Model_AppReleasesMapperBase
{
	protected $_dbTable;
	...
	public function getDbTable()
	{
		if (null === $this->_dbTable) {
			$this->setDbTable('Default_Model_DbTable_AppReleases');
		}
		return $this->_dbTable;
	}

	public function save(Default_Model_AppRelease $value)
	{
		global $application;
		$data = array();
		if ( ! isnull($value->getId()) ) $data['id'] = $value->getId();
		if ( ! isnull($value->getAppID()) ) $data['appid'] = $value->getAppID();
		if ( ! isnull($value->getRelease()) ) $data['release'] = $value->getRelease();
		if ( ! isnull($value->getSeries()) ) $data['series'] = $value->getSeries();
		...
		if ( ! isnull($value->getManagerID()) ) $data['manager'] = $value->getManagerID();
		if ( ! isnull($value->getLastupdated()) ) $data['lastupdated'] = $value->getLastupdated();
		if ( ! isnull($value->getReleaseID()) ) $data['releaseid'] = $value->getReleaseID();
	...
	public function delete(Default_Model_AppRelease $value)
	{
		$q1 = 'id = ?';
		$q2 = $value->id;
		$s=$this->getDbTable()->getAdapter()->quoteInto($q1,$q2);
	...
	public function populate(&$entry,$row)
	{
		...
		$entry->setRelease($row->release);
		$entry->setSeries($row->series);
		$entry->setState($row->state);
		$entry->setAddedon($row->addedon);
		$entry->setPublishedon($row->publishedon);
		$entry->setManagerID($row->manager);
		$entry->setLastupdated($row->lastupdated);
		$entry->setReleaseID($row->releaseid);
	}

	public function find($id, Default_Model_AppReleases &$value)
	{
	...
	public function fetchAll($filter = null, $format = '')
	{
		...
		foreach ($resultSet as $row) {
			$entry = new Default_Model_AppRelease();
