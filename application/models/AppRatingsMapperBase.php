...
class Default_Model_AppRatingsMapperBase
{
	protected $_dbTable;
	...
	public function getDbTable()
	{
		if (null === $this->_dbTable) {
			$this->setDbTable('Default_Model_DbTable_AppRatings');
		}
	...
	public function save(Default_Model_AppRating $value)
	{
		...
		if ( ! isnull($value->getRating()) ) $data['rating'] = $value->getRating();
		if ( ! isnull($value->getComment()) ) $data['comment'] = $value->getComment();
		if ( ! isnull($value->getSubmittedOn()) ) $data['submittedon'] = $value->getSubmittedOn();
		if ( ! isnull($value->getSubmitterID()) ) $data['submitterid'] = $value->getSubmitterID();
		if ( ! isnull($value->getSubmitterName()) ) $data['submittername'] = $value->getSubmitterName();
		if ( ! isnull($value->getSubmitterEmail()) ) $data['submitteremail'] = $value->getSubmitterEmail();
	...
	public function delete(Default_Model_AppRating $value)
	{
		$q1 = 'id = ?';
		$q2 = $value->id;
		$s=$this->getDbTable()->getAdapter()->quoteInto($q1,$q2);
	...
	public function populate(&$entry,$row)
	{
		...
		$entry->setSubmittedOn($row->submittedon);
		$entry->setSubmitterID($row->submitterid);
		$entry->setSubmitterName($row->submittername);
		$entry->setSubmitterEmail($row->submitteremail);
		$entry->setGuID($row->guid);
		$entry->setModerated($row->moderated);
	}

	public function find($id, Default_Model_AppRatings &$value)
	{
