...
class Default_Model_OSesMapperBase
{
	protected $_dbTable;
	...
	public function getDbTable()
	{
		if (null === $this->_dbTable) {
			$this->setDbTable('Default_Model_DbTable_OSes');
		}
		return $this->_dbTable;
	}

	public function save(Default_Model_OSe $value)
	{
		global $application;
		$data = array();
		if ( ! isnull($value->getId()) ) $data['id'] = $value->getId();
		if ( ! isnull($value->getName()) ) $data['name'] = $value->getName();
		if ( ! isnull($value->getOSFamilyID()) ) $data['os_family_id'] = $value->getOSFamilyID();
	...
	public function delete(Default_Model_OS $value)
	{
		$q1 = 'id = ?';
		$q2 = $value->id;
		$s=$this->getDbTable()->getAdapter()->quoteInto($q1,$q2);
	...
	public function populate(&$entry,$row)
	{
		...
		$entry->setOSFamilyID($row->os_family_id);
	}

	public function find($id, Default_Model_OSes &$value)
	{
