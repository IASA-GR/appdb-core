...
class Repository_Model_CommRepoOssMapperBase
{
	protected $_dbTable;
	...
	public function getDbTable()
	{
		if (null === $this->_dbTable) {
			$this->setDbTable('Repository_Model_DbTable_CommRepoOss');
		}
	...
	public function save(Repository_Model_CommRepoOs $value)
	{
		...
		if ( ! isnull($value->getDisplayName()) ) $data['displayName'] = $value->getDisplayName();
		if ( ! isnull($value->getFlavor()) ) $data['flavor'] = $value->getFlavor();
		if ( ! isnull($value->getDisplayFlavor()) ) $data['displayFlavor'] = $value->getDisplayFlavor();
		if ( ! isnull($value->getArtifactType()) ) $data['artifactType'] = $value->getArtifactType();
	...
	public function delete(Repository_Model_CommRepoOs $value)
	{
		$q1 = 'id = ?';
		$q2 = $value->id;
		$s=$this->getDbTable()->getAdapter()->quoteInto($q1,$q2);
	...
	public function populate(&$entry,$row)
	{
		...
		$entry->setDisplayName($row->displayName);
		$entry->setFlavor($row->flavor);
		$entry->setDisplayFlavor($row->displayFlavor);
		$entry->setArtifactType($row->artifactType);
		$entry->setAcronym($row->acronym);
	}

	public function find($id, Repository_Model_CommRepoOss &$value)
	{
	...
	public function fetchAll($filter = null, $format = '')
	{
		...
		foreach ($resultSet as $row) {
			$entry = new Repository_Model_CommRepoOs();
