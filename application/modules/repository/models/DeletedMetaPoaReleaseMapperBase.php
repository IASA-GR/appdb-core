...
class Repository_Model_DeletedMetaPoaReleaseMapperBase
{
	protected $_dbTable;
	...
	public function getDbTable()
	{
		if (null === $this->_dbTable) {
			$this->setDbTable('Repository_Model_DbTable_DeletedMetaPoaRelease');
		}
		return $this->_dbTable;
	}

	public function save(Repository_Model_DeletedMetaPoaRelease $value)
	{
		global $application;
		$data = array();
		...
		if ( ! isnull($value->getProductReleaseId()) ) $data['productReleaseId'] = $value->getProductReleaseId();
		...
		if ( ! isnull($value->getPoaUrl()) ) $data['poaUrl'] = $value->getPoaUrl();
		if ( ! isnull($value->getPoaCandidateUrl()) ) $data['poaCandidateUrl'] = $value->getPoaCandidateUrl();
		if ( ! isnull($value->getPoaPath()) ) $data['poaPath'] = $value->getPoaPath();
		if ( ! isnull($value->getTargetPlatformCombId()) ) $data['targetPlatformCombId'] = $value->getTargetPlatformCombId();
		if ( ! isnull($value->getDMethodCombId()) ) $data['dMethodCombId'] = $value->getDMethodCombId();
	...
	public function delete(Repository_Model_DeletedMetaPoaRelease $value)
	{
		$q1 = 'id = ?';
		$q2 = $value->id;
		$s=$this->getDbTable()->getAdapter()->quoteInto($q1,$q2);
	...
	public function populate(&$entry,$row)
	{
		...
		$entry->setId($row->id);
		$entry->setProductReleaseId($row->productReleaseId);
		$entry->setDisplayVersion($row->displayVersion);
		$entry->setReleaseNotes($row->releaseNotes);
		$entry->setChangeLog($row->changeLog);
		$entry->setPoaUrl($row->poaUrl);
		$entry->setPoaCandidateUrl($row->poaCandidateUrl);
		$entry->setPoaPath($row->poaPath);
		$entry->setTargetPlatformCombId($row->targetPlatformCombId);
		$entry->setDMethodCombId($row->dMethodCombId);
	...
	public function find($id, Repository_Model_DeletedMetaPoaRelease &$value)
	{
	...
	public function fetchAll($filter = null, $format = '')
	{
		...
		foreach ($resultSet as $row) {
			$entry = new Repository_Model_DeletedMetaPoaRelease();
