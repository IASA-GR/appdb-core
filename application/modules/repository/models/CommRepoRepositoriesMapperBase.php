...
class Repository_Model_CommRepoRepositoriesMapperBase
{
	protected $_dbTable;
	...
	public function getDbTable()
	{
		if (null === $this->_dbTable) {
			$this->setDbTable('Repository_Model_DbTable_CommRepoRepositories');
		}
		return $this->_dbTable;
	}

	public function save(Repository_Model_CommRepoRepository $value)
	{
		global $application;
		$data = array();
		if ( ! isnull($value->getId()) ) $data['id'] = $value->getId();
		if ( ! isnull($value->getName()) ) $data['name'] = $value->getName();
		if ( ! isnull($value->getBasePath()) ) $data['base_path'] = $value->getBasePath();
		if ( ! isnull($value->getBaseUrl()) ) $data['base_url'] = $value->getBaseUrl();
	...
	public function delete(Repository_Model_CommRepoRepository $value)
	{
		$q1 = 'id = ?';
		$q2 = $value->id;
		$s=$this->getDbTable()->getAdapter()->quoteInto($q1,$q2);
	...
	public function populate(&$entry,$row)
	{
		...
		$entry->setBasePath($row->base_path);
		$entry->setBaseUrl($row->base_url);
	}

	public function find($id, Repository_Model_CommRepoRepositories &$value)
	{
	...
	public function fetchAll($filter = null, $format = '')
	{
		...
		foreach ($resultSet as $row) {
			$entry = new Repository_Model_CommRepoRepository();
