...
class Repository_Model_CommRepoStatesMapperBase
{
	protected $_dbTable;
	...
	public function getDbTable()
	{
		if (null === $this->_dbTable) {
			$this->setDbTable('Repository_Model_DbTable_CommRepoStates');
		}
		return $this->_dbTable;
	}

	public function save(Repository_Model_CommRepoState $value)
	{
		global $application;
		$data = array();
		if ( ! isnull($value->getId()) ) $data['id'] = $value->getId();
		if ( ! isnull($value->getName()) ) $data['name'] = $value->getName();
		if ( ! isnull($value->getRepositoryId()) ) $data['repository_id'] = $value->getRepositoryId();
	...
	public function delete(Repository_Model_CommRepoState $value)
	{
		$q1 = 'id = ?';
		$q2 = $value->id;
		$s=$this->getDbTable()->getAdapter()->quoteInto($q1,$q2);
	...
	public function populate(&$entry,$row)
	{
		...
		$entry->setRepositoryId($row->repository_id);
	}

	public function find($id, Repository_Model_CommRepoStates &$value)
	{
	...
	public function fetchAll($filter = null, $format = '')
	{
		...
		foreach ($resultSet as $row) {
			$entry = new Repository_Model_CommRepoState();
